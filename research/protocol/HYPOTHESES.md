...
- **v7.0** is a convergent high-vol trend/momentum book whose ~26–32pp
...
- **v3.4** is a consistency-first book (1/24 negQ, worst quarter −1.4%) whose
...
They are structurally complementary — v3.4's stress-payers should cushion
exactly the tail that caps v7.0's leverage, and v7.0's explosive trend capture
should lift exactly the CAGR/Sharpe ceiling that v3.4's reweighting studies
...
channels are dead (H8/H14/H15). The federation level is genuinely untested.
...
One account, capital split w to v7.0 book / (1−w) to v3.4 book at t0; each
book compounds its own sub-capital with its native mechanics (v7 band
re-splits operate on v7 slot equities only; v3.4 fractions size on v3.4
...
- Pre-registered grid: w ∈ {0.30, 0.40, 0.50, 0.60, 0.70} at each book's
  native scale (v7 @ R8-anchor extraction, v3.4 @ scale 10). No off-grid picks.
...
- Mandatory: fixed-schedule ablation on any F2 winner; coupling perturbation
  ...
  state (v7 band gap clock does NOT reset on federation re-splits).

## H-FED-3 — Scale re-pick on the winning structure (LAST lever)

Mechanical rule, committed now: on the winning federation config, sweep global
...
Re-derive the two v3.4 hard limits for the combined book: overnight |XAUUSD|
...
the v3.4 "structural rule beats fitted pin" doctrine). This lever can only
...
would then justify: v3.4's crisis sleeve weight ×{1.5, 2.0} INSIDE the v3.4
sub-book (weight moved from cash-park, NOT from other sleeves; total v3.4
gross unchanged ≤ its own convention). This touches a parent weight — it is
the ONLY such lever licensed, it is conditional, and it uses v3.4's own
...
kill); carrying FMA2 sleeves as band slots or the band inside v3.4 (closed
