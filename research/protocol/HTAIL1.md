...
Ledger: FMA3-006. One lever: the Satellite sub-book's crisis sleeve weight is
...
(17.97% native). Crisis is Satellite's native stress-payer (H14: pays on non-band
...
stress-payer in the Satellite residual attacks dDD/dw exactly where it binds.
...
1. Rebuild the Satellite' book: `combine(sleeves, weights_m) × 10` +
   `apply_hard_limits` (FMA2 machinery, read-only).
2. Run Satellite' ALONE in the record engine (€10k) → its native curve B'_m and
   own metric block (reported for context; no bar on it).
3. Blend at w = 0.70 with A (Core native, unchanged) and B'_m,
