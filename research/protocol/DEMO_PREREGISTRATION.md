...
for the FMA3 federation. It **extends** [docs/v1.0/DEMO.md](../../docs/v1.0/DEMO.md)
...
**Live/demo execution of the two-stack federation reproduces the record-engine
...
|---|---|---|
| v7 stack `InpRisk` | 8 × 0.70 × s = **5.6·s** | 6.16 |
| v3.4 stack `GLOBAL_SCALE` | 10 × 0.30 × s = **3.0·s** | 3.30 |
...
lines; account id, v7 preset-file checksum, FMA2 re-stamped deploy hash.
...
- **Bar (pre-registered in ROADMAP v1.1):** retention **≥ 0.85** of
  record-engine CAGR.
- **Precedent and the honest unknown:** v7 alone at matched R8 retained
  **~96%** of Python in the MT5 tester (Python 89.7% → MT5 86.5%,
  `NSF5 docs/v7/PERFORMANCE.md` — *observed, not guaranteed*). **The
  federation's retention is UNKNOWN** — the v3.4 stack has never had a tick
...
|---|---|---|---|---|---|---|
| Federation (w=0.70, **s=1.1**) | +6.18% | 6.15% | 10/72 | −10.69% (2022-01) | +19.91% (2025-10) | **[−10.69%, +22.02%]** |
| v7 sub-book (native **R8** anchor) | +5.90% | 6.90% | 13/72 | −9.22% (2022-05) | +22.92% (2021-03) | **[−11.89%, +23.68%]** |
| v3.4 sub-book (native **scale-10** pin) | +5.72% | 7.82% | 14/72 | −17.14% (2022-01) | +22.83% (2024-11) | **[−17.14%, +25.87%]** |

Forward cross-check (same script): federation +14.94 / −0.25 / +0.41 / −2.42%
(window +12.34%); v3.4 sub +15.50 / +5.06 / −3.36 / −3.13% (window +13.59%) —
reproduces [FORWARD_ONESHOT.md](../outputs/FORWARD_ONESHOT.md) and DEMO.md
exactly. January 2026's +14.94% sits inside the federation band — a
front-loaded month is in-fingerprint.

- **Scaling rule (pre-committed):** the federation band scales by
...
- **Return bar:** a sub-book monthly return outside its (scaled) 99% band →
  §4 P1 pause. The federation month outside its band → logged; two consecutive
  → §4 P2.
...
- **Trade-count fingerprint:** counting conventions differ across the three
  ledgers (25,869 record-engine fills ≈ 1,078/quarter federation-wide; v7
  ~468 round-trips/yr; v3.4 ~2,286 model-trades/yr —
...
- **Scope:** the v3.4 stack only — `InpMakerFirst` lives in
  `FableExecutor.mq5`; the v7 EA has no maker path (always taker; its spread
...
- **Flip gate (FMA2 `docs/v3.4/DEMO.md` §5.2, adopted verbatim + federation
  ...
  met. Federation additions: earliest flip after **two full monthly ledger
...
criteria, F2_PREMORTEM guard pattern) applied at the federation level. Freed
capital from any pause/kill **parks in cash — never renormalize into the
survivor** (FMA2 governance rule, adopted federation-wide in DEMO.md rule 4).
Inherited hard layers stay live underneath: OPS-9a close-mark −25%
flatten-and-halt on joint equity (2 marks 5 min apart), v7 `InpMarginCap=0.9`,
v3.4 60% de-gross, both parents' own DEMO/RUNBOOK sleeve rules. (P2 note: the
...
| ID | Observable | Threshold | Action |
|---|---|---|---|
| **P1** | Sub-book monthly return outside its 99% band (B3, scaled) | one month outside [−11.89%, +23.68%]·(s*/1.0 on native basis) for v7 or [−17.14%, +25.87%]·s* for v3.4 (reference at native dials; addendum instantiates) | pause **that stack's** new entries; investigate under its parent's own DEMO/H12 rules; other stack untouched; freed capital to cash. Resume on written benign adjudication; second consecutive out-of-band month after resume → treat as fingerprint drift (P2) with the stack still paused. |
...
- A losing month inside its band; a losing quarter on the v3.4 slice (an
  explicit DEMO.md watch item — it runs above its standalone ceiling under the
  federation's license); a vol-upshock day or cluster (NSF5 FM-1: expected
...
Precedent for why: v7.0's COVID tail measured **35.6%** on MT5 real-tick vs
...
1. **Source runs:** the MT5 Strategy Tester real-tick run(s) of the federation
   on the owner's machine over **2020–25** at the deployed dials — the v7 EA
   directly; the v3.4 stack via its first-ever tester/tick reconciliation
   (FMA2 §C OPEN — if the v3.4 tester harness is not ready at deploy, k is
   measured first on the v7 stack + the live demo curve, and the federation k
   is completed when the v3.4 tick run exists; the interim asymmetry is
...
long enough to span at least one v7 band-cadence cycle and, ideally, one vol
spike.
...
No book change from anything the demo shows (weights, sleeves, w, federation
