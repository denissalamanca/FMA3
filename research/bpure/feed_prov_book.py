"""FMA3 blend propagation of the Duka-hybrid v34 feed.

static_blend: fed[h,k] = f_core*(w*a/j) + f_sat*((1-w)*b/j), j = w*a + (1-w)*b, w=0.70.
Swapping the v34 feed changes BOTH f_sat (build_c2 positions) and b (v34 standalone
equity). f_core and a (v7) are unchanged. We recompute fed with hybrid f_sat+b and
...
def fed_from(sat_frac: pd.DataFrame, b_curve: pd.Series, w: float = W) -> pd.DataFrame:
    """Replicate M.static_blend but with injected sat_frac + b (v34 equity multiple).
    core_frac and a (v7 equity) come from the frozen pin inputs."""
    core_frac, _sat_frac_pin, a, _b_pin = M.load_inputs()
...
if __name__ == "__main__":
    # PIN fed (canonical)
    fed_pin = M.static_blend(W)

    # hybrid inputs
    sat_frac_hyb = pd.read_parquet(SCRATCH / "pos_hybrid.parquet")
    b_hyb = pd.read_parquet(SCRATCH / "hybrid_curve.parquet")["equity"]
    fed_hyb = fed_from(sat_frac_hyb, b_hyb)
