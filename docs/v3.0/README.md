...
| **[SLEEVE_REGIME_ANALYSIS.md](SLEEVE_REGIME_ANALYSIS.md)** | 📊 **Trade characteristics & performance by sleeve and regime** — the deep read of the current model-of-record run: per-symbol trade stats, Core-vs-Sat sleeve roles (standalone alpha, ρ=0.109 diversification, capital tilt), the shared-symbol attribution caveat, and the 2020-2025 regime-by-regime narrative. **Standing battery doc — regenerate each update.** |
| **[FTMO_DIAL_DECISION.md](FTMO_DIAL_DECISION.md)** | 🎚️ **FTMO dial decision — s≈0.70 under a ≤1-breach/year policy** (native-EA runs A/B, the breach-frequency-vs-s curve, the monthly-withdrawal rationale). Supersedes the "shipped FTMO dial unsafe / cut to s≈0.35" read, which assumed a zero-breach standard. |
| **[FTMO_WEIGHT_PROBE.md](FTMO_WEIGHT_PROBE.md)** | 🧪 **FTMO ±20% weight-probe — mechanism parity with IC, not assurance parity** (FMA3-011 + adversarial adjudication). The dial is robust to ±20% *weight* drift, but the probe is *pass-by-construction* and *frame-blind* (score_v3's monthly reset scores an >10% static DD as compliant); the binding lever is the *dial s* (+20% → ~2 breaches/yr), an independent reason to prefer s≈0.65. The native-EA static-frame crisis probe remains the open arbiter. |
| **[DEMO_FORWARD_PLAN.md](DEMO_FORWARD_PLAN.md)** | 🚀 **Demo-forward charter + production checklist** — IC + FTMO demo accounts on the owner's VPS: purpose, pre-committed success/kill criteria, 3-month window, the warm-start prerequisite, the logging spec (audit), the reconciliation harness, and the 1-week disabled → enabled rollout. The demo produces the **first valid OOS read**. |
