...
FMA3 v1.0 is the shipped blend book — **both frozen parent books side by
side as virtual sub-accounts of ONE cross-margined €10k account**: the NSF5 Core
band book at capital share **w = 0.70** and the FMA2 Satellite fixed-fraction book at
...
Static blend w70/30 (H-FED-1 winner by rule) · no cross-book rebalance
...
| Doc | What it is |
|---|---|
| **[STRATEGY.md](STRATEGY.md)** | The "what & why" — the two frozen sub-books, the exact blend bookkeeping formula, the fresh-seed/no-rebalance convention, the anti-coupling guard (±€128 probe), how w = 0.70 and s = 1.1 were picked by pre-registered rule, and everything tried and declined. |
| **[PERFORMANCE.md](PERFORMANCE.md)** | The canonical performance read — official pin, both gate scoreboards, yearly/quarterly/monthly tables, the parents in one accounting, measured blend friction (−2.7pp), the scale frontier and the probe-robust adjudication, breach bootstrap, the MT5↔1m crisis-tail gap, the forward one-shot. |
...
| **[RECONCILIATION.md](RECONCILIATION.md)** | The three-engines-one-accounting map — verdict `reconciled`: 41/41 + curves 0.0, 15/15 + 9/9 legs bit-exact, ext engine 38/38 bit-identical, pin rebuild delta 0.0; the measured translation costs (Core native €532,230 vs record €492,611; −2.7pp friction; 35.6% vs 5.54% tick↔1m tail) and the owed MT5 arbiter. |
...
| Doc | What it is |
|---|---|
| **[../whitepaper/00_WHITEPAPER.md](../whitepaper/00_WHITEPAPER.md)** | The Fable Book — executive summary, document map, definition of done. |
...
| **[../whitepaper/02_FEDERATION_DESIGN.md](../whitepaper/02_FEDERATION_DESIGN.md)** | Blend mechanics — the bookkeeping formula, scale-invariance, the anti-coupling guard, the pre-registered evaluation ladder. |
