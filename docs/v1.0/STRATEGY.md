...
The authoritative "what + why" for the locked FMA3 v1.0 federation book.
...
federation mechanics and the pre-registered ladder in
...
virtual sub-accounts — the NSF5 v7.0 band book (7 slot-equity sleeves, `BAND_SYM_25` re-split +
H9 delta-resize) at capital share `w = 0.70`, and the FMA2 v3.4 fixed-fraction book (8 sleeves ×
...
|---|---|---|
| **Record engine (official pin)** | static federation w=0.70, s=1.1, IC 2020-25, €10k | **CAGR +101.4% / 15.73% worst-mark DD / Sharpe 2.467 / COVID tail 5.36% / 0 negY / 0 negQ / breach P(DD>30%) 0.0020 / €10k → €665,777** |
...
| MT5 real-tick | — | **never run for the federation** — the deployable arbiter, pending on the owner's machine |
...
live in one engine: the record-engine 5.36% must never be quoted against v7's MT5 real-tick
35.6% — the 1m↔tick gap is documented in
[COMPOSITE_BENCHMARK.md](../../research/outputs/COMPOSITE_BENCHMARK.md), and the federation's
...
space — capital split `w`, federation rebalance mechanics, combined-book exposure limits, global
...
Federation is the *one* open channel because every sleeve-level path between the parents is
...
| NSF5 sleeves → v3.4 | **CONSUMED** | the one-shot 2015-19 OOS gauntlet is spent |

The lesson those closures teach — sleeve value is cadence/structure-conditional — is why the
federation **changes neither architecture**: each book keeps its own mechanics on its own
...
daily-return **ρ = +0.351** between the books, **drawdown troughs disjoint** (v3.4 at 0.2% DD
during v7's trough 2021-05-23; v7 at 3.9% during v3.4's trough 2022-02-10), and each book's
worst year is the other's relative refuge (2022: v3.4 +32.1% vs v7 +55.6%, both books' worst).
Quoted honestly: v3.4 returns **−2.9%** on average across v7's ten worst days — a *softener,
not a hedge*.
...
| Sub-book | Sleeves | Native mechanics (untouched) | Native anchor (byte-verified) | Record-engine reference (gate basis) | Seed share | Share of federation P&L |
|---|---|---|---|---|---|---|
| **v7.0 band book** (NSF5) | 7 | slot-equity sleeves, 1/7 each; `BAND_SYM_25` re-split (share > 0.25 / < 0.0816, 5d min-gap) + H9 delta-resize; R8 anchor extraction | `engine_reproduce.json:harvest_band_sym`, cagr_bd 0.8972 — extraction 15/15 anchor floats delta 0.0 | **+91.5% / 21.22% DD / Sharpe 2.267 / 0 negQ** | **0.70** | **~73.5%** |
| **v3.4 book** (FMA2) | 8 | fixed-fraction × `GLOBAL_SCALE` 10, F3 caps, cash-park doctrine, 2 structural hard limits; config hash `48c09199fbf83d82` | `v34_s10_pin_1m.json`, cagr 0.8866 — wrapper 41/41 checks delta 0.0 | **+88.7% / 21.67% DD / Sharpe 1.854 / 1 negQ** | **0.30** | **~26.5%** |

- The record-engine reference rows are the **gate basis** — only v3.4@s10 (pin) and v7@r8
  (exact) are gate-grade; v7 r9/r10 rows are linear approximations and set no gates.
- P&L shares are w-weighted native-curve growth (v7 multiple 53.2×, v3.4 45.0× on €10k;
...
## 4. The federation mechanics (in full)
...
- `fracV7` — v7.0's held-exposure hourly fraction-of-book-equity matrix from the
  byte-reconciled anchor re-run (`v7_book_frac_1h.parquet`);
- `fracV34` — the shipped v3.4 book exactly as its official pin constructs it
...
not assumed** — the realized-vs-ideal drift is reported as federation friction on every grid
...
asymmetry is disclosed: the v7 leg replays held-exposure snapshots with a **~1-bar execution lag**
vs its native engine; the cost is priced into the measured v7@r8 record profile, so parent
references and federation candidates are compared like-for-like inside one accounting
...
- **v7.0's band triggers are slot *ratios*** — `share[n] = slot_equity[n] / Σ slots` crossing
  0.25 / 0.0816 is invariant to scaling all slots by a constant; sleeve sizing is inverse-vol on
  slot equity. Running the v7 book on `w·€10k` produces the identical re-split dates and the
  native path scaled by `w`.
- **v3.4's positions are equity *fractions*** — its return path is independent of starting
...
any federation bookkeeping must isolate each book's internal trigger state from the other book's
P&L. In the static federation this holds **by construction** — the blend inputs are the parents'
...
| Probe (v7 sub-book seed) | Δ final equity | Δ max DD | Tolerance | Verdict |
|---|---|---|---|---|
| **+€128** | **+0.32%** | **+0.01pp** | \|Δfinal\| < 5%, \|ΔDD\| < 1pp | ✅ within |
| **−€128** | **−1.10%** | **−0.02pp** | \|Δfinal\| < 5%, \|ΔDD\| < 1pp | ✅ within |

**PASS — the federation is chaos-stable**, in sharp contrast to the overlay-ring's −€59k. A
corollary rule carried from H-FED-2: v7's internal band min-gap clock never resets on any
federation-level event.
...
| w (v7 share) | CAGR | Max DD (worst-mark) | Sharpe | COVID tail | negY / negQ | All bars |
|---|---|---|---|---|---|---|
...
diversification the leverage dial cannot buy (re-levering v7 r8→r10 costs +5.0pp DD and a negQ
for +30.6pp CAGR). Measured federation friction at w70: **−2.70pp CAGR** (ideal bookkeeping
...
static federation keeps the trough disjointness intact — **static w70 stands**
([REGISTRY.md FMA3-002](../REGISTRY.md)). This also removes an entire failure class: with no
federation rebalance schedule there is nothing for schedule-chaos to couple to (the
...
three points). Because a never-rebalanced federation's realized w **drifts** (measured band
...
FAIL is a smooth frontier toward the measured v7-alone endpoint (DD 21.22% at w = 1.0), not a
...
dimensions beat the dimension-wise best of both parents (below 1.1, CAGR misses the v7@r8 91.5%
...
Each sub-book's structural limits arrive **pre-applied inside its own fraction matrix**: v3.4's
overnight |XAUUSD| ≤ 1.80×E_sub and managed-cross ≤ 0.5×E_sub (plus its F3 per-sleeve caps and
cash-park), and v7's per-sleeve caps. The open question M-0 flagged was *stacking*: both books
...
|---|---|---|---|---|---|---|
| Overnight \|XAUUSD\| | 1.10×E | 1.75×E | 1.97×E | **2.03×E** | **2.03×E** (= \|v7 own gold\|×share + 1.80×v3.4 share) | **0** |
| Managed crosses (EURCHF/EURSEK/EURNOK/AUDNZD) | — | — | — | 0.187–0.191×E | 0.1945×E (0.5×E×v3.4 share; v7 trades none of them) | **0** |
...
unnecessary — which is itself evidence the federation does not manufacture hidden leverage.
...
| `w_v7` | **0.70** | v7.0 band-book capital share, grid winner by rule (§5) |
| `global_scale` | **1.1** | H-FED-3 ceiling rule + FMA3-RT probe-robust re-pick (§7) |
| parent A operating point | v7.0 core7 band, **R8 anchor extraction** | `v7_book_frac_1h.parquet` from the byte-reconciled anchor (15/15 delta 0.0) |
| parent B operating point | v3.4 @ **GLOBAL_SCALE 10** | `books.py::build_v34_frac_1h()` = the FMA2 pin construction (41/41 delta 0.0) |
...
- **Crisis-tail numbers live in one engine.** Record-engine tail 5.36% vs v7's MT5 real-tick
  35.6% is an engine-granularity gap, not a risk reduction; the federation's tick-level tail is
...
- **Federation friction is real: −2.7pp CAGR** at the locked point (ideal 92.4% vs realized
  89.7% at s = 1.0) — min-lot quantization at €7k/€3k 2020 sub-books, joint margin, netting;
  measured by the engine, not assumed away.
- **The v7 leg carries a ~1-bar execution lag** vs its native engine (held-exposure snapshot
...
- **Forward-honest Sharpe is not 2.47.** By the parents' own convention the budgeted band is
  **1.2–1.5** (v3.4's disclosed ratio); the 2026H1 one-shot printed **1.17** on 4 months —
...
- **The 2026H1 CONFIRM is a proxy-book confirmation, not a reconciliation**: USA500 proxies
  USTEC (corr 0.89), v3.4 ran at 0.88× reduced breadth on 14-symbol Duka coverage, Duka ≠ IC
...
| `python3 engine/v7_bridge/run_extract.py` (~1 min) | v7 fraction matrix = the byte-reconciled NSF5 anchor | **15/15 anchor floats delta 0.0** |
| `python3 scripts/verify_record_engine.py` (~6–8 min) | record engine reproduces the FMA2 v3.4 pin | **41/41 checks delta 0.0**, curve max-abs-delta 0.0 |
