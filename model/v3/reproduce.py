...
This is the SINGLE source of truth for the federated model behind
docs/v1.0/DASHBOARD_IC.html and DASHBOARD_FTMO.html. It is self-contained: it
inlines the federation blend (so it does NOT depend on the scattered
...
DO NOT confuse this model with:
  - hrisk1_results.json 'v7-alone'  -> WRONG, hrisk1 IS federated (blends f7+f34).
...
This model = static_fed(w=0.70) * s, s per preset (IC 1.6, FTMO 0.7). See MODEL_SPEC.md.
"""
...
FRAC7 = REPO / "research/outputs/v7_book_frac_1h.parquet"          # v7 band-book hourly frac-of-own-equity (8 legs)
EQ7   = REPO / "research/outputs/v7_book_equity_1m.parquet"        # v7 native standalone 1m equity ("eqc")
EQ34  = REPO / "research/baselines/fma2/v34_s10_pin_curve.parquet" # v34 native standalone 1m equity ("equity")
# frac34 = books.build_v34_frac_1h()  (31 cols, GLOBAL_SCALE=10, gold cap 1.80 pre-applied)

W_V7 = 0.70                 # v7 capital share (hashed in config 51a7541cc2aaa593)
...
def load_inputs():
    ...
    frac7 = pd.read_parquet(FRAC7)
    frac34 = books.build_v34_frac_1h()
    eq7 = pd.read_parquet(EQ7)["eqc"]
    eq34 = pd.read_parquet(EQ34)["equity"]
    return frac7, frac34, eq7 / eq7.iloc[0], eq34 / eq34.iloc[0]


def static_fed(w: float) -> pd.DataFrame:
    """The federation blend: fed[h,k] = f7*(w*a_h/j) + f34*((1-w)*b_h/j),
    ...
    Byte-identical to scripts/run_hrisk1.py::static_fed (verified -> €3,872,872)."""
