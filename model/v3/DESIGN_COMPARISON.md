...
1. **The v7 book is path-dependent.** Its band re-split + seed-chain is internal state, not a function of the latest bars. In **Python** there is no one-bar step (the repo's single sanctioned `NotImplementedError`) → interim is a ~10–25 min full re-extraction *every hour*. **In MQL5 this is already solved** — `V7Core.mqh` *is* that incremental stepper, G1-proven to the cent (€398,368.75).
2. **The v34 book is the opposite.** Its 8-sleeve pandas alpha is a big, fidelity-fragile port to any non-Python language — but it **already runs live in Python**: the FMA2 brain writes hourly targets bit-identical to the pin (6.66e-16), and the EA already consumes them (`V34Live.mqh`, InpV34Mode=2). Live v34 is essentially *done*.
...
**Reading 1+2 together:** each language is strong exactly where the other is weak. v7 is easy in MQL5 / hard in Python; v34 is easy in Python / hard in MQL5. A design that forces *both* books into one language pays the port tax on one of them. That is the whole thesis.
...
|---|---|---|---|
| **v7 forward** | ✗ Python re-sim ~10–25 min/hr | ✓✓ V7Core stepper (solved) | ✗ Python re-sim ~10–25 min/hr |
| **v34 forward** | ✓ Python brain (live, proven) | ✗✗ port 1400-line alpha; may never reach parity | ✓ Python brain (live, proven) |
...
| **Strategy-Tester validation gate** | ✓ (MQL5 tester) | ✓✓ dynamic, no-CSV (v7 half now) | ✗ lost — forward-only validation, weeks not minutes |
| **Fragile handoff** | ✗ full-book CSV append race + tail-reader (new, unbuilt) | ✓ none (but blocked on v34 wall) | ~ no CSV, but the bot *is* the trader |
| **Deploy substrate** | terminal + Python | terminal only | Windows/Wine + terminal + Python (MetaTrader5 is Windows-only) |
| **Net new risky code** | v7 re-sim service + CSV race + FedReplay tail-reader | V7Sim `a_h` + **v34 alpha port** + 3 in-proc ledgers | exec port + watchdog/resync + v7 re-sim + GuardianEA |
| **Honest floor it degrades to** | itself | **v7-native + v34-Python-bridge** (still a hybrid for half) | itself + GuardianEA |

**What each is actually good for:**
- **A** is the general fallback, but pays the v7 Python-re-sim tax and owns a brand-new fragile CSV-append/tail-reader race — the exact fragility you flagged.
- **B pure** dies on the v34 wall: a bit-perfect pandas→MQL5 port still misses the pin because it runs a *different feed*, and it may never reach parity while the v34 sleeves are still being tuned. Its **v7-native tester mode**, however, is a genuine low-risk win.
...
## 3. Recommendation — D: v7-native (MQL5) + v34-brain-bridge (Python) + on-terminal EA
...
- **Kills the v7 blocker** (the hardest upstream item in A and C) — V7Core already steps forward natively; no Python re-extraction, no lock_v5 isolation, no splice-seed re-base for v7.
- **Avoids the v34 wall** (B's killer) — no pandas→MQL5 alpha port; uses the brain that already produces v34 live, bit-identical to the pin.
- **Keeps the on-terminal EA** (safety + the Strategy-Tester gate) that C throws away.
- **Shrinks the fragile handoff from the whole book to v34-only — over a mechanism that already exists and is proven** (`V34Live.mqh` consuming the brain's `targets.json`). This is *not* A's new, unbuilt full-book CSV-append/tail-reader race; it's the shipped live-v34 path.
- **Net new risky code is the smallest of any option:** essentially `V7Sim` (V7Core forked to bookkeep an idealized standalone account → `a_h`) + the blend + the native-equity seed. The v34 side is reused.

**What D honestly costs (no papering over):**
- It is **still a Python→file hybrid for the v34 half** — the exact shape you object to, just minimized and over a proven channel rather than a new one. If "zero Python in the live loop" is a hard requirement, only B-pure satisfies it, and B-pure is blocked on the v34 port that may never close.
...
- `b_h` (v34 native equity) must be produced somewhere — cleanest is a small Python shadow stepping `account_engine_1m._run_chunk` alongside the brain (so the v34 bridge carries `f34`+`b_h`), which the maps rate *incremental and tractable*.
...
2. **Cheap go/no-go first (from B's plan §8.1):** measure the feed-provenance number — run the v34 brain on broker-exported bars vs the frozen cache, propagate through `static_fed`, and put a real number on "how far does live-feed v34 land from the pin." This gates *every* design and costs a day.
3. **P0 — V7Sim tester spine:** fork V7Core → idealized standalone account → `a_h`/`f7`, run it dynamically in the tester at position level (B's genuine win, low-risk). Proves the v7 half with no CSV.
4. **P1 — v34 bridge + `b_h` shadow:** wire the existing brain live-path; add the small Python `b_h` stepper; blend in the EA; reconcile same-feed.
...
**Fallbacks, ranked:** if the v34-brain live-wiring proves fragile, D degrades to **A** (full CSV) with the v7 half still native — strictly better than A-as-specified. If "no Python live" becomes a hard mandate, the only door is **B-pure**, gated on the v34 port passing parity (do not start it while the v34 sleeves are still being tuned). **C** is not recommended for a live-money path: it discards the tester gate and the on-terminal backstop for a code-reuse win that D captures without those sacrifices.
