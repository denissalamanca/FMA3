...
that accounting is verified at **delta 0.0 or bit-identical**: the Satellite pin reproduces 41/41 +
minute curves 0.0, the Core anchor extraction matches 15/15 + 9/9 legs bit-exact, the
...
record accounting are **measured and attributed, not open**: the Core native-vs-record equity gap
(€532,230 vs €492,611) is the record engine's ~1-bar execution lag plus its cost/margin
conventions; blend assembly costs a measured −2.7pp CAGR of real friction; and the
MT5↔1m crisis-tail gap is priced at 35.6% vs 5.54% for Core alone. Zero unexplained mismatches
...
blend itself — is explicitly not claimed here and is listed below as the owed arbiter.
...
|---|---|---|
| **Native band engine** (NSF5) | `gbandrebal/sim.run_generic`, R8, IC 2020-25, idealized close-and-reseed, bd-return accounting | Core anchor: **CAGR 89.72% (bd) / 15.70% bd-DD / 19.44% tick-DD / Sharpe 2.58 / €532,230 / 31 band + 0 harvest triggers** (`engine_reproduce.json:harvest_band_sym`, byte-reconciled) |
| **Native FMA2 1m engine** | `research/account_engine_1m.py::simulate_account_1m` — one balance, one margin pool, worst-intrabar mark, full IC cost realism | Satellite pin @ s10: **CAGR +88.66% / 21.67% worst-mark DD / Sharpe 1.854 / €449,708 / 20,403 trades / 1 negQ (2023Q1 −1.42%)** (`v34_s10_pin_1m.json`) |
| **Record engine** (FMA3 engine of record) | the FMA2 1m engine via the verified `record_engine.py` wrapper — Satellite runs *identically by construction*; the Core book is replayed through it via the extracted per-1m-bar position matrix | Core @ r8 in record accounting: **+91.5% / 21.22% / Sharpe 2.267 / €492,611 / 6,406 trades / 0 negQ**; the blend pin: **+101.4% / 15.73% / 2.467 / €665,777** ([composite_benchmark.json](../../research/outputs/composite_benchmark.json), [fma3_v1_pin.json](../../research/outputs/fma3_v1_pin.json)) |
| **MT5 real-tick** (deployable arbiter — **not yet run for the blend**) | `PortfolioV7.mq5` R10 in the NSF5 program; nothing exists for Satellite or the blend | Core-only native headline: 96.1% CAGR / 20.9% Maximal eq-DD / **35.6% COVID Relative DD** / Sharpe 2.03 — quoted for the tail gap in (B3), never mixed with record numbers |
...
### B1. Core native → record: **€532,230 vs €492,611** (~1-bar lag + conventions)

The Core leg enters the blend as a held-exposure snapshot: the extraction re-ran the exact
...
| Core @ R8, same book | Native band engine | Record engine (r8, exact) |
|---|---|---|
...
This cost applies **identically to parent references and blend candidates** — every
...
`COMPOSITE_BENCHMARK.md` draft printed the native €532,251 figure in the Core@r8 record row; the
...
### B2. Blend assembly friction: **−2.7pp CAGR**, priced not assumed
...
inside every reported number — the blend's +9.9pp CAGR over the best parent is *net* of it.

### B3. The MT5 ↔ 1m crisis-tail gap: **35.6% vs 5.54%** (Core-only measured, standing for the campaign)
...
| Core COVID crisis tail (2020-02-15 → 04-15, peak-to-trough) | Value |
|---|---|
...
same-engine comparisons only; (2) the blend's MT5 run on the owner's machine remains the
deployable arbiter of the tail; (3) Core's −21.8% MT5 2020Q1 is a real-tick artifact of this
...
| Link | Check | Result |
|---|---|---|
| Parents' native anchors (REGISTRY [FMA3-000](../REGISTRY.md)) | Satellite pin re-run in FMA2's engine; Core anchor re-run via NSF5 `v7val/tier12.py` | Satellite pin **byte-identical** (byte-reproduced twice); Core tier12 **byte PASS** (89.72% / €532,230 / 31 triggers) |
| Record-engine wrapper ([verify_record_engine.json](../../research/outputs/verify_record_engine.json)) | 41 metric checks + minute-level equity/worst curves vs the pinned Satellite reference | **41/41 delta 0.0**; curve max-abs-delta **0.0** (index match true) |
...
1. **No MT5 run of the blend book — the owed arbiter.** The record engine is a 1m-bar
   worst-mark model; the deployable truth is MT5 real-tick on the owner's machine, and the
   blend has never been run there. The Core-only measured tail gap (35.6% vs 5.54%, B3) shows
   how large the translation *can* be in a crisis window; the blend's tick-granularity tail
   is therefore **unknown by construction**. Satellite has never had a tick run at all — its 21.67%
...
2. **The 2026H1 forward book is a directional confirmation, not a reconciliation.** It ran on
   the Duka feed (documented ~8pp CAGR_bd 2020-25 divergence vs IC), 14-symbol coverage (Satellite at
...
3. **The owner's six original gates still straddle two engines** (secondary scoreboard). The
   blend's 5.36% record-engine tail clears the 35.6% MT5-derived bar only in the trivial
...
1. **€532,230 vs €492,611 (Core native vs record)** — ~1-bar execution lag + cost/margin
   conventions; measured, attributed, applied like-for-like to every comparison (B1).
2. **−2.7pp blend friction** — real min-lot/margin/netting costs of one account; priced by
...
cross-engine gaps (~1-bar lag €532,230→€492,611, −2.7pp blend friction, 35.6%-vs-5.54%
tick↔1m tail) measured and disclosed rather than open. Zero unexplained mismatches;
`reconciled`. The MT5 real-tick run of the blend on the owner's machine is the one owed
