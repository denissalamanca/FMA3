...
stacks, side by side, on ONE cross-margined demo account), how the locked blend config maps onto
...
[`strategy_fma3.py`](../../strategy_fma3.py) (static blend w=0.70 / s=1.1, config hash
...
Sibling docs (all `docs/v1.0/`): [STRATEGY](STRATEGY.md) (what the blend is) ·
...
one cross-margined €10k account — 70% the Core band book (R8 anchor extraction), 30% the Satellite
...
| F4 | each sub-book window return > −20% | Core **+15.99%**, Satellite **+13.59%** | PASS |
...
**The honest operational headline: there is no FMA3 EA.** The blend is operationally **two
parent EA stacks plus a capital split** — the Core EA (self-contained MQL5) and the Satellite stack (Python
...
2. **Market Watch — the union of both universes, IC-native names.** Core stack: `XAUUSD, USTEC,
   USDJPY, ETHUSD, EURGBP, BTCUSD` + S6 legs `USDJPY/AUDUSD/NZDUSD` + conversion `EURUSD, EURJPY`
   (⚠️ USTEC, not US500 — `InpUS500=USTEC`). Satellite stack: the 37-instrument universe of FMA2
...
3. **Core stack (the 70% share).** Compile `FableMultiAsset1_V7.mq5` in MetaEditor (magic base
...
4. **Satellite stack (the 30% share).** Follow FMA2 `ea/RUNBOOK.md` §2 in order — brain offline sanity,
...
5. **Magic-number separation — verify, don't assume.** Core stack: 360000-range. Satellite stack: magic
   band **8400001–8400008**, one per sleeve, `mag_xau=8400008` (FMA2 `docs/v3.4/DEMO.md` §2; note
   their `RUNBOOK.md` §1.2 still prints the older v3-era 920001–920007 map — the Satellite DEMO band is
...
6. **Day-1 checks:** every open position's magic resolves to exactly one stack; Core health file shows
   `volume_rejects=0`; FMA2 kill-switch `NONE`, heartbeats fresh on the watchdog, no
   `config_hash_mismatch`.
7. **Record in the track record:** deploy date, account id, the Core preset file (and its checksum),
   the FMA2 stamped deploy hash, and the FMA3 config hash `51a7541cc2aaa593` they jointly implement.

Both stacks keep their own margin defenses live: Core `InpMarginCap=0.9`; Satellite in-strategy 90% margin
...
The pinned blend matrix is `fed_frac = frac7·(w·A/J) + frac34·((1−w)·B/J)`, all `× s`
...
|---|---|---|---|---|---|
| Core band book | `InpRisk = 8` (the R8 anchor extraction, `strategy_fma3.py::parents.v7`; Python cagr_bd 89.7%, byte-reconciled) | × 0.70 | × 1.1 | **`InpRisk = 6.16`** on the full €10k | €7,000 sub-account at R = 8×1.1 = **8.8** (6.16 × 10,000 = 8.8 × 7,000) |
| Satellite book | `GLOBAL_SCALE = 10` (their locked re-pick) | × 0.30 | × 1.1 | **`GLOBAL_SCALE = 3.30`** on the full €10k | €3,000 sub-account at scale 10×1.1 = **11** (3.3 × 10,000 = 11 × 3,000) |

Three honest notes on those dials:

- **The Core dial reads below the parent's "never run V7 below R8" floor (NSF5 `docs/v7/DEMO.md`) — by
  ...
  this configuration.
- **The Satellite slice runs at an 11-equivalent per sub-capital — above its own standalone ceiling.**
  Their pre-registration rejected scale 11 for failing the negQ gate standalone. The blend pins
...
- **Hard limits rescale with the dial.** Satellite structural caps are 0.18×scale (overnight gold) and
...
Fallback point, pre-derived: stepping the blend to **s=1.0** means `InpRisk = 5.60` and
...
2. **No virtual sub-account wall.** The pin's construction keeps each book blind to the other's P&L
   (`strategy_fma3.py`, PROTOCOL §5.7). Live: the Core EA compounds its own internal sleeve seeds
   (`InpInitial`-seeded — a good approximation of the convention), but the Satellite brain's targets are
   *fractions of account equity* (FMA2 `ea/RUNBOOK.md` §1) — the executor reads **joint** equity, so
   the Satellite slice's capital base drifts with Core's P&L. This shared-equity coupling is exactly the
...
3. **FMA2's guards now see joint equity.** Its DD/HWM guards (−25% flatten-and-halt OPS-9a, −18%
   ...
   direction, and adopted here as the blend's kill layer (§Decision rules). But its **€8,000
...
4. **No joint watchdog.** The FMA2 watchdog covers only its own comms files; the Core EA has no
...
5. **The FMA2 EA stack's open hardening items carry over verbatim.** Tick-side reconciliation is
   **OPEN** — no MT5 Strategy Tester run has ever been performed for Satellite, and no live/demo tick has
...
7. **The blend has never run on real ticks.** The pin is 1m Python; the forward CONFIRM was
...
| Trade cadence | ~**1,078 trades/quarter ≈ 83/week** blend-wide (pin total; the per-stack split is not pinned — establish it from live magic attribution in week 1) | either stack silent for days, or trade counts a multiple of the joint cadence |
...
| **Sub-book balance** | long-run growth share **Core ~73.5% / Satellite ~26.5%**; forward window natives +15.99% vs +13.59% (Satellite sub-path +15.50/+5.06/−3.36/−3.13%) | a sub-book < **−20%** on a rolling 4-month native basis (the F4 bar), or realized w outside **0.56–0.84** (the ±20% probe envelope) |
| **Margin** | forward F3 measurements: max margin/balance **0.324** (cap 0.90), min margin level **311%** (stop-out 50%) | margin/balance > **0.60** sustained (the Satellite de-gross zone, now on joint equity), or ML trending below ~**200%** — the survival signal, not the DD % |
...
| Core stack internals | per NSF5 `docs/v7/DEMO.md`: band re-splits ~5–9/yr with `nSlots=7 / floor≈0.08`, per-sleeve mix (gold ~43% of *stack* P&L), JPY *earns* swap, EURGBP takes shorts | NSF5's red flags apply unchanged to the 70% slice |
| Satellite stack internals | per FMA2 `docs/v3.4/DEMO.md` §3 + RUNBOOK §8: seasonal slippage ≤ **0.8 bp** (20-day rolling — their single most important demo output), fill rate ≥ **98%**, forced exits 21:05/06:05 every session, mag_xau *banks the drift* | FMA2's red flags apply unchanged to the 30% slice; any unexplained `HALT_FLATTEN` |
| **`volume_rejects` / rejects** | **0 in both stacks** (Core health file; FMA2 fill ledger) | any nonzero → stop and diagnose (sizing/min-lot at the *smaller* sub-scales is the first suspect — the 3.3-scale slice trades smaller lots than anything FMA2 ever demoed) |

**Where the numbers live:** Core stack — `portfolio_v7_decisions.csv`, `portfolio_v7_health.csv`,
heartbeat/state CSVs. Satellite stack — `broker_snapshot.json`, `slippage_ledger.csv`,
...
4. **F4 analog — one sub-book < −20% on a rolling 4-month native basis:** investigate **that stack
   ...
   governance rule, adopted blend-wide).
...
- **Everything validated is in-sample (IC 2020-25), and the one shot at 2026H1 is spent.** The
  forward CONFIRM was a Duka-feed proxy book (USA500 for USTEC, 14 symbols, Satellite at 0.88×-reduced
...
- **The Satellite slice runs above its own standalone ceiling** (11-equivalent per sub-capital vs their
  scale-10 pre-registration). Its license is the blend pin (negQ 0/24) + the ±20% probes + the
...
- **Both parents' tail caveats carry over, rescaled.** Core's corr-spike and fat-tail×leverage risks
  scale with the dial; Satellite's MKT-3 single p95-hour joint gap (−28.1% standalone at scale 10) maps
  to ≈ **−9.3% of joint equity** at the 3.3 dial (linear-in-scale approximation) and its intraday
  gold stack remains uncapped within the slice. Diluted by blend, not removed.
...
w from magic attribution (Core 360000-range vs Satellite 8400001–8400008 across
`broker_snapshot.json` + the Core decision/health CSVs), joint worst-mark DD vs the envelope, ML and
margin/balance vs the F3 measurements; **(b) Core stack** — the NSF5 Monday health read
(`portfolio_v7_health.csv`: `volume_rejects`, `final_ML`; band cadence + per-sleeve fingerprint from
the decisions log); **(c) Satellite stack** — FMA2 RUNBOOK §7 daily/weekly checklists (slippage ledger
...
reconciliation demands. Blend-specific: script the joint monitor (realized w, joint DD, joint
...
- Both stacks track their parent fingerprints: Core band cadence ~5–9/yr with nSlots=7 and healthy
  sleeve mix; Satellite seasonal slippage ≤ 0.8 bp, fill rate ≥ 98%, mag_xau drift-capture calibrated
