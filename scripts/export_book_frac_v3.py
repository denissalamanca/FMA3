#!/usr/bin/env python3
"""Export the UNIFIED book_frac replay stream for FableFederation_V3.

Emits the netted static_blend(0.70) matrix (33 symbols, the model/v3 stable model)
...
Format (fmt=3, simpler than the v34 fmt=2 — pure position replay, no sleeve schedule):
  line 1 : core_weight=0.70,config_hash=51a7541cc2aaa593,fmt=3
...
Self-checks (hard-fail):
  * re-parse -> matrix reproduces static_blend(0.70) to < 1e-12 (always).
...
import reproduce as M   # canonical static_blend / load_inputs / targets / CONFIG_HASH / CORE_WEIGHT
...
def write_csv(path: Path, rows) -> None:
    ...
    with open(path, "w") as fh:
        fh.write(f"w_v7={M.CORE_WEIGHT},config_hash={M.CONFIG_HASH},fmt={FMT}\n")
        for e, sym, v in rows:
            if sym == "__GRID__":
...
def reparse(path: Path, index: pd.DatetimeIndex, model_cols) -> pd.DataFrame:
    ...
    with open(path) as fh:
        ...
        assert kv["config_hash"] == M.CONFIG_HASH and kv["fmt"] == str(FMT) and float(kv["w_v7"]) == M.CORE_WEIGHT, hdr
        for line in fh:
            f = line.rstrip("\n").split(",")
            e = int(f[0])
...
def main() -> int:
    ...
    # --- fast self-check: re-parse reproduces static_blend to < 1e-12 ---
