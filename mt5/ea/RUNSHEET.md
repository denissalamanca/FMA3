...
(only the TRANSPLANT_V7.md allowlist differs from the Core source). Re-run it + the §Rebuild
...
blend bookkeeping vs the `strategy_fma3` construction incl. the anti-coupling property,
...
`python3 scripts/export_sat_replay.py --install` — must print OVERALL PASS; record the new
sha256 here next to the run that consumes it.

---

## G1 — Core parity vs run 54 (RUN THIS FIRST)
...
2. **Market Watch:** `XAUUSD, USTEC, USDJPY, ETHUSD, EURGBP, BTCUSD, AUDUSD, NZDUSD,
   EURUSD, EURJPY` (Satellite symbols not needed — the replay file is not even opened at
   `InpV34Mode=0`).
3. **Strategy Tester:**
   | Setting | Value |
   |---|---|
...
4. **Startup checks (Journal/Experts):** `SLOT-EQUAL over 7 slots … slotW=0.1429`;
   `F3 init: v34mode=0 … v7=on fed=off`; no `F3 REPLAY` lines (Satellite off).
5. **PASS criteria:**
...
- **G2b (long):** preset **`FED_G2_V34ONLY_S10.set`**, window **2020-01-02 → 2025-12-31**,
  ...
  the Satellite k — the COVID tail is the measurement, not a gate).
...
3. **PASS (from `Common\Files\fma3_fed_books.csv` daily rows):**
   ...
   - every `REBAL` row in `fma3_fed_decisions.csv` has book equity (`extra` col) equal to
     the same-day `E_v7` books value — **no Satellite P&L in any Core seed**;
   - the Core `REBAL`/`BAND` **dates match the G1 run's dates exactly** (anti-coupling; a
...
4. **Report read (not pass/fail):** `python3 scripts/combine_tester_reports.py --v7 …`
   conventions vs the s=1.6 blend record (hrisk1: CAGR 170.2%, maxDD 22.58%, tail
   8.12%) → the blend k; `w_realized` drift-band watch (review trigger outside
   0.56–0.84).

## G3b — blend run, COVID-fixed EA (SUPERSEDES G3) — READY 2026-07-10
...
- **COVID silence fixed:** all-flat in-index hours now emit `__GRID__` sentinels (`fmt=2` CSV)
  so the Satellite book actively **flattens** instead of holding stale targets — Satellite trades again
...
blend real-tick DD/tail → blend-level k → final IC dial. Do **not** run this until RUN 1's
...
- [ ] **Satellite-distinctive fills EXIST inside Feb–Apr 2020** — XAGUSD / XBRUSD / US30 / XTIUSD /
...
4. **Seam 4.1 placement corrected** vs TRANSPLANT_V7.md §3: the `InpEnableV7=false` weight
   ...
   placement would have left S6+BTC trading at half-book each in Satellite-only mode.
...
7. **`InpV34SymbolMap` (2026-07-10, owner fix):** new blend input, default
...
- **G2a replay + symbol map: PASS** on ICMarketsEU-Demo (851013 rows, hash 51a7541cc2aaa593, USA500→US500, DAX→DE40, Satellite trades fire).
- **BLOCKER for G2b/G3 — MT5 history for 8 Satellite symbols (31% of the book):**
...
- **Unblock next session:** (1) restart MT5 (clears the 6 cached ones);
  ...
  then G3 (FED_IC.set) for the blend k → final IC dial.
...
- Demo lacked history for 8 Satellite symbols (31% of book). SWITCHING TO THE REAL
  ...
  'every tick generation' from M1 bars (same as Core runs 53/54).
- **G2a PASS (real acct):** full Satellite book traded, 851013 rows, hash match,
  symbol map (USA500->US500, DAX->DE40) applied, 0 volume_rejects, Jan-2021
  smoke final EUR 11,285.
- **G3 RUNNING:** FED_IC.set (Core+Satellite blend, s=1.6, guardian off), 2020-01..2025-12,
  real ticks, EUR 10k. This is the deliverable → blend real-tick DD/tail →
  blend-level k → final IC dial re-pick. Compare vs record pin: CAGR +101.4%*/DD
...
- NOTE: cross-server consistency — Core runs 53/54 were on demo; G3 on real. Minor
  spec differences; the blend k is taken from G3 (self-contained, both books
