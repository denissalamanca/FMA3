...
//| Include/Sat/MagXau.mqh (CSatMagXauStepper).                  |
//|                                                                  |
...
void OnStart()
  {
   ...
   //================================================================
   CSatMagXauStepper st;
   long day0 = SatMAGXAU_DAY_NS * (long)20000;   // arbitrary day boundary
   int  bars = 0;
   int  nonzero1 = 0;
   for(int d = 0; d < 4; d++)
     {
      for(int h = 0; h < 8; h++)
        {
         long ts = day0 + (long)d * SatMAGXAU_DAY_NS + (long)h * CHK_HOUR_NS;
         double c;
   ...
   for(int h = 0; h < 6; h++)
     {
      long ts = wday + SatMAGXAU_DAY_NS + (long)h * CHK_HOUR_NS;
   ...
   for(int k = 0; k < 20; k++)
     {
      long ts = wday + SatMAGXAU_DAY_NS + (long)6 * CHK_HOUR_NS
   ...
   double pdt = w.Step((datetime)((wday + (long)3 * SatMAGXAU_DAY_NS)
   ...
   bool ok = (nonzero1 == 0) && (mismatches == 0)
             && SatIsObs(firstpos) && (firstpos > 0.0);
