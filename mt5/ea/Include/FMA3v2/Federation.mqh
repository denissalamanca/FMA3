...
//====================================================================
// [F3 v2 CAUSE A] quote-currency-correct EUR conversion for v34-only symbols
//====================================================================
// The v1 EurPerQuote handles only USD/JPY(as USDJPY)/GBP(as EURGBP); every other
// v34 quote currency fell to the 1/EURUSD catch-all -> JPY crosses ~117x under,
// NOK/SEK ~10x under -> below min-lot -> 7 symbols never traded. These helpers
// (called from V7Core::EurPerQuote, forward-declared in the .mq5) fix that path
// WITHOUT touching v7: F3_IsV7Symbol routes all 9 v7 legs to the verbatim block.
bool F3_IsV7Symbol(string sym)
  {
   return( sym==InpXAU||sym==InpUS500||sym==InpUSDJPY||sym==InpETH
        || sym==InpEURGBP||sym==InpAUD||sym==InpEURUSD||sym==InpNZD||sym==InpBTC );
  }
// EUR-cross symbol for a v34 quote currency (conversion only; not traded).
string F3_EurCrossFor(string q)
  {
...
// Correct EUR-per-quote for the v34-only symbols. PRIMARY = 1/MidOf(EUR-cross) to
// match the engine of record (record_engine_ext._eurq_chunk); FALLBACK = terminal
// tick-value primitive (unit_eur = px*tick_value/tick_size identity; deposit ccy
// EUR); else 0.0 -> DesiredLots' eurq<=0 guard fires F3_SizeSkipWarn (loud, never
// a silent mis-size).
double F3_EurPerQuoteV34(string sym)
  {
   string q=SymbolInfoString(sym,SYMBOL_CURRENCY_PROFIT);
   if(q=="EUR") return 1.0;                                          // DE40/DAX: no cross (mirrors _eurq_chunk q==EUR)
   if(q=="USD"){ double m=MidOf(InpEURUSD); return (m>0)?1.0/m:0.0; }// == old ELSE for USD v34 legs (US500)
   string cross=F3_EurCrossFor(q);                                  // PRIMARY: model-faithful EUR-cross MID
   if(cross!=""){ double m=MidOf(cross); if(m>0.0) return 1.0/m; }
...
//====================================================================
// [F3 v2 CAUSE B] v34 sizing base = joint-account share (model fed_frac (1-w)*B/J)
//====================================================================
// v1 sizes v34 off its OWN sub-equity e34, which grew 4x while the account grew
// 24x -> e34/account collapsed 0.87->0.15 -> chronic under-sizing (US500 ~0.11x).
// The MODEL sizes v34 off its share of the JOINT account: fed_frac v34 = (1-w)*B/J
// (scripts/run_hfed1.py), the (1-w) already carried by InpV34Mult=(1-w)*s. Return
// acct*E_v34/(w*E_v7+(1-w)*E_v34). Gated on InpV34JointSizing AND g_f3FedActive
// (v34-only J=(1-w)*e34 would inflate the base by 1/(1-w)); OFF/degenerate -> e34.
double F3_V34SizingBase()
  {
   double e34=F3_V34BookEquity();
   if(!(InpV34JointSizing && g_f3FedActive)) return e34;           // default OFF or v34-only -> byte-identical
   double J=InpWv7*F3_V7BookEquity()+(1.0-InpWv7)*e34;             // joint account (same denom as the books log row)
   double acct=AccountInfoDouble(ACCOUNT_EQUITY);
   if(J<=0.0 || acct<=0.0) return e34;                            // degenerate -> safe fallback
   return e34*acct/J;                                             // = acct*E_v34/(w*E_v7+(1-w)*E_v34)
  }
