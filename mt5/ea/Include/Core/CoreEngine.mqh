//+------------------------------------------------------------------+
//| Core/CoreEngine.mqh - the VERBATIM v7 transplant (FableFederation_V1) |
//|                                                                    |
...
{
   // [F3 v2 CAUSE A] v7 FROZEN fast-path: the original 3 branches VERBATIM for every
   // v7 traded symbol (F3_IsV7Symbol), and for ALL symbols when the fix is inert
   // (InpV34EurQuoteFix=false) -> bit-identical to v1. Only v34-only symbols the v1
   // catch-all mis-priced (non-USD/JPY/GBP quote ccy) reach F3_EurPerQuoteV34.
   if( !InpV34EurQuoteFix || F3_IsV7Symbol(sym) )
   {
      double m;
   ...
   return F3_EurPerQuoteV34(sym);                    // v34-only corrected path (cause A)
}
