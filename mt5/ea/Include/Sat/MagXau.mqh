...
//|     (SatBankerRound, same tie test as the Python).               |
//|  3. dist      : (d - near) / STEP                                |
...
//|                                                                  |
//| STATE (SSatMagXauState) mirrors the Python state dict            |
...
//|   current                     -> current                         |
//+------------------------------------------------------------------+
#ifndef SAT_MAGXAU_MQH
#define SAT_MAGXAU_MQH

#include <Sat/SatMath.mqh>

//--- module constants (verbatim from mag_xau_stepper.py) -----------
const string SatMAGXAU_SYM     = "XAUUSD";
const double SatMAGXAU_STEP    = 100.0;
const double SatMAGXAU_BAND    = 0.18;
const double SatMAGXAU_MIND    = 0.03;
const double SatMAGXAU_OFFSET  = 0.0;
const double SatMAGXAU_SIDE    = 1.0;
const double SatMAGXAU_VT      = 0.15;
const double SatMAGXAU_CAP     = 6.0;
const int    SatMAGXAU_VOL_WIN = 20;
// DAY_NS = 86_400_000_000_000 (built by product: literal-safe)
const long   SatMAGXAU_DAY_NS  = (long)86400 * (long)1000000000;

//------------------------------------------------------------------//
// SSatMagXauState — field-for-field mirror of the Python state     //
// dict (get_state / from_state).  NaN encodes Python None for the  //
// nullable doubles; has_accum_day encodes `accum_day is None`.     //
//------------------------------------------------------------------//
struct SSatMagXauState
  {
   int               version;        // == 1
...
//------------------------------------------------------------------//
// CSatMagXauStepper — see header comment for the API contract.     //
//------------------------------------------------------------------//
class CSatMagXauStepper
  {
public:
   ...
   double            StepNs(const long ts_ns, const double close_raw)
     {
      long day = (ts_ns / SatMAGXAU_DAY_NS) * SatMAGXAU_DAY_NS;
   ...
   void              FlushFinalDay()
     {
      if(m_has_accum_day && SatIsObs(m_accum_close))
         FinalizeDay();
     }

   //--- EA state (mirrors get_state / from_state) -------------------
   void              GetState(SSatMagXauState &out) const
     {
      out.version       = 1;
      out.sleeve        = "mag_xau";
   ...
   void              SetState(const SSatMagXauState &in)
     {
      Reset();
   ...
   void              FinalizeDay()
     {
      ...
      double near_lvl = (SatBankerRound(mid / SatMAGXAU_STEP - SatMAGXAU_OFFSET)
                         + SatMAGXAU_OFFSET) * SatMAGXAU_STEP;
      double dist = (mid - near_lvl) / SatMAGXAU_STEP;
      double sig;
      if(SatMAGXAU_SIDE > 0.0)
         sig = (dist < -SatMAGXAU_MIND && dist > -SatMAGXAU_BAND) ? 1.0 : 0.0;
      else
         sig = (dist > SatMAGXAU_MIND && dist < SatMAGXAU_BAND) ? -1.0 : 0.0;
      ...
      if(n >= SatMAGXAU_VOL_WIN + 1)
        {
         double mean = 0.0;
         int    base = n - SatMAGXAU_VOL_WIN - 1;
         double rets[];
         ArrayResize(rets, SatMAGXAU_VOL_WIN);
         for(int i = 0; i < SatMAGXAU_VOL_WIN; i++)
           {
            double r = m_mids[base + i + 1] / m_mids[base + i] - 1.0;
            rets[i] = r;
            mean += r;
           }
         mean /= (double)SatMAGXAU_VOL_WIN;
         double ss = 0.0;
         for(int i = 0; i < SatMAGXAU_VOL_WIN; i++)
           {
         ...
         ann = MathSqrt(ss / (double)(SatMAGXAU_VOL_WIN - 1))
               * MathSqrt(252.0);
        }
      else
         ann = SatNan();
      ...
      if(!SatIsObs(ann))
      ...
      else if(ann == 0.0)
        {
         ...
         // 0/0 -> NaN
         double sv = sig * SatMAGXAU_VT;
         if(sv == 0.0)
            tgt = SatNan();
         else
            tgt = (sv > 0.0) ? SatMAGXAU_CAP : -SatMAGXAU_CAP;
        }
      else
        {
         tgt = sig * SatMAGXAU_VT / ann;
         if(tgt > SatMAGXAU_CAP)
            tgt = SatMAGXAU_CAP;
         else if(tgt < -SatMAGXAU_CAP)
            tgt = -SatMAGXAU_CAP;
        }
      ...
      m_pend_ts[p]  = day + SatMAGXAU_DAY_NS;
      m_pend_tgt[p] = tgt;
