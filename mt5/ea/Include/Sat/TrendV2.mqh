//+------------------------------------------------------------------+
//| TrendV2.mqh — FMA3 v34 trend_v2 sleeve stepper (CSatTrendV2Stepper)
//|
...
//|          ignore_na=False  -> CSatEwmMean (exact aggregations.pyx
...
//| SatMath header): pandas' compiled ewma kernel contracts
...
//|
//| API:
//|   CSatTrendV2Stepper st;               // ctor == Python __init__
...
//+------------------------------------------------------------------+
#ifndef SAT_TRENDV2_MQH
#define SAT_TRENDV2_MQH
...
//------------------------------------------------------------------//
#define SatTV2_NSYM 5
#define SatTV2_NLB  6

string SatTV2_SYMS[SatTV2_NSYM] =
  {"XAUUSD", "XAGUSD", "XBRUSD", "XTIUSD", "XNGUSD"};
int    SatTV2_LOOKBACKS[SatTV2_NLB] = {15, 25, 40, 65, 95, 125};

#define SatTV2_K         1.0
#define SatTV2_DELTA     0.15
#define SatTV2_S0        0.15
#define SatTV2_VOL_SPAN  20
#define SatTV2_VOL_MINP  10
#define SatTV2_V0        0.085
#define SatTV2_XAG_SHARE 0.5
#define SatTV2_EXEC_HOUR 5      // held effective d+1 05:00 UTC (lag 6h)
#define SatTV2_MAX_L     125
#define SatTV2_XAG_IDX   1      // SYMS[1] == "XAGUSD"

//------------------------------------------------------------------//
// np.tanh semantics: NaN->NaN, +-inf->+-1, else libm tanh.          //
// (explicit guards so MQL5's MathTanh corner behavior is irrelevant)//
//------------------------------------------------------------------//
double SatTV2Tanh(const double z)
  {
   if(z != z)
      return SatNan();
   double inf = SatInf();
...
//------------------------------------------------------------------//
string SatTV2Num(const double x)
  {
   if(x != x)
      return "NaN";
   if(!SatIsFinite(x))
...
int SatTV2JsonValuePos(const string js, const string key)
  {
...
int SatTV2JsonFlatArray(const string js, int &pos, double &vals[])
  {
   ...
   ArrayResize(vals, n);
   for(int i = 0; i < n; i++)
      vals[i] = SatParseDouble(toks[i]);   // handles NaN/Infinity tokens
   return n;
  }

// scalar number (int-valued fields parse through double exactly)
double SatTV2JsonNumber(const string js, const string key, bool &ok)
  {
   int p = SatTV2JsonValuePos(js, key);
   if(p < 0)
     {
      ok = false;
      return SatNan();
   ...
   ok = true;
   return SatParseDouble(StringSubstr(js, p, e - p));
  }

//==================================================================//
// CSatTrendV2Stepper — steps ALL 5 symbols together, one calendar- //
// day close row per call.  Exact port of TrendV2Stepper.           //
//==================================================================//
class CSatTrendV2Stepper
  {
public:
   ...
   CSatRing          m_hist[SatTV2_NSYM];
   // pandas-exact ewma of dret^2 per symbol (span=20, minp=10);
   // CSatEwmMean fields m_avg/m_old_wt/m_nobs == Python
   // ewm_weighted/ewm_old_wt/ewm_nobs
   CSatEwmMean       m_ewm[SatTV2_NSYM];
   double            m_held[SatTV2_NSYM];
   long              m_n_rows;

   // --- last-step intermediates (Python .last, diagnostics/parity) --
   double            m_last_sig_d[SatTV2_NSYM];
   double            m_last_s[SatTV2_NSYM];
   double            m_last_max_w[SatTV2_NSYM];
   double            m_last_target[SatTV2_NSYM];
   bool              m_last_moved[SatTV2_NSYM];

                     CSatTrendV2Stepper() { Reset(); }

   //---------------------------------------------------------------//
   void              Reset()
     {
      double nan = SatNan();
      for(int i = 0; i < SatTV2_NSYM; i++)
        {
         m_hist[i].Init(SatTV2_MAX_L);
         m_ewm[i].Init((double)SatTV2_VOL_SPAN, SatTV2_VOL_MINP);
         m_held[i]        = 0.0;
         m_last_sig_d[i]  = nan;
         m_last_s[i]      = nan;
         m_last_max_w[i]  = nan;
   ...
   int               NSymbols() const { return SatTV2_NSYM; }
   string            SymbolName(const int i) const { return SatTV2_SYMS[i]; }
   ...
   void              Step(const double &closes[], double &held_out[])
     {
      ...
      for(int i = 0; i < SatTV2_NSYM; i++)
        {
         ...
         // CSatEwmMean == the pandas aggregations.pyx recurrence the
         ...
         double legs[SatTV2_NLB];
         for(int j = 0; j < SatTV2_NLB; j++)
           {
            int L = SatTV2_LOOKBACKS[j];
            ...
            if(c == c && p_l == p_l)
               ...
               num = nan;
            // _ieee_div == SatNpDiv: NaN/inf per IEEE, no fp divide by 0
            double z = SatNpDiv(num, sig * MathSqrt((double)L));
            legs[j] = SatTV2Tanh(z / SatTV2_K);
           }
         ...
         double sgn_s = SatSign(s);
         double agree_cnt = 0.0;
         for(int j = 0; j < SatTV2_NLB; j++)
           {
            if(SatSign(legs[j]) == sgn_s)     // NaN==NaN false -> adds 0
               agree_cnt += 1.0;
           }
         ...
         double a = MathAbs(s) - SatTV2_S0;   // NaN propagates
         if(a < 0.0)
            a = 0.0;                          // NaN stays NaN (cmp false)
         s = (SatSign(s) * a) / (1.0 - SatTV2_S0);
         m_last_s[i] = s;

         // ---- inverse-vol sizing with per-instrument cap ------------
         double mw = SatNpDiv(SatTV2_V0, ann_vol); // ann_vol==0 -> inf
         if(mw > 1.0)
            mw = 1.0;                         // clip(upper=1): NaN stays
         if(i == SatTV2_XAG_IDX)              // SYMS[i] == "XAGUSD"
            mw = mw * SatTV2_XAG_SHARE;
         m_last_max_w[i] = mw;
         ...
         double band = SatTV2_DELTA * (SatIsFinite(mw) ? mw : 1.0);
         m_last_moved[i] = false;
         if(SatIsFinite(tgt) && MathAbs(tgt - m_held[i]) > band)
           {
            m_held[i] = tgt;
            m_last_moved[i] = true;
           }
      ...
      m_n_rows++;
      ArrayResize(held_out, SatTV2_NSYM);
      for(int i = 0; i < SatTV2_NSYM; i++)
         held_out[i] = m_held[i];
     }
         ...
         for(int k = 0; k < cnt; k++)
           {
            if(k > 0)
               js += ", ";
            js += SatTV2Num(m_hist[i].TrimmedAt(cnt, k));  // oldest->newest
           }
         js += "]";
        }
      js += "], \"ewm_weighted\": [";
   ...
   bool              SetState(const string js)
     {
      ...
      if(StringFind(js, "\"trend_v2\"") < 0)
         return false;
      int sp = SatTV2JsonValuePos(js, "syms");
      if(sp < 0)
         return false;
      ...
      bool ok = true;
      double n_rows_d = SatTV2JsonNumber(js, "n_rows", ok);
      if(!ok || !(n_rows_d == n_rows_d))
         return false;

      // hist: 5 nested flat arrays inside the outer [ ... ]
      int hp = SatTV2JsonValuePos(js, "hist");
      if(hp < 0)
         return false;
      ...
      if(outer < 0)
         return false;
      int    cursor = outer + 1;
      double hist_vals[SatTV2_NSYM][SatTV2_MAX_L];
      int    hist_cnt[SatTV2_NSYM];
      for(int i = 0; i < SatTV2_NSYM; i++)
        {
         double vals[];
         int n = SatTV2JsonFlatArray(js, cursor, vals);
         if(n < 0 || n > SatTV2_MAX_L)
            return false;
         hist_cnt[i] = n;
         for(int k = 0; k < n; k++)
      ...
      double w[], ow[], nb[], hd[];
      int p;
      p = SatTV2JsonValuePos(js, "ewm_weighted");
      if(p < 0 || SatTV2JsonFlatArray(js, p, w) != SatTV2_NSYM)
         return false;
      p = SatTV2JsonValuePos(js, "ewm_old_wt");
      if(p < 0 || SatTV2JsonFlatArray(js, p, ow) != SatTV2_NSYM)
         return false;
      p = SatTV2JsonValuePos(js, "ewm_nobs");
      if(p < 0 || SatTV2JsonFlatArray(js, p, nb) != SatTV2_NSYM)
         return false;
      p = SatTV2JsonValuePos(js, "held");
      if(p < 0 || SatTV2JsonFlatArray(js, p, hd) != SatTV2_NSYM)
         return false;
