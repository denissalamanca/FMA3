//+------------------------------------------------------------------+
//| SatMath.mqh — FMA3 v34 shared numeric primitives                 |
//|                                                                  |
...
//+------------------------------------------------------------------+
#ifndef SAT_SATMATH_MQH
#define SAT_SATMATH_MQH
...
bool SatIsNan(const double x)   { return (x != x); }
bool SatIsObs(const double x)   { return (x == x); }

// math.isfinite: not NaN and not +-inf
bool SatIsFinite(const double x)
  {
   if(x != x) return false;
   double inf = SatInf();
   return (x < inf && x > -inf);
...
bool SatSignBit(const double x)
  {
   SatDoubleBits u;
   u.d = x;
   return (u.l < 0);
...
//------------------------------------------------------------------//
double SatNpDiv(const double a, const double b)
  {
   if(a != a || b != b)
      return SatNan();
   if(b == 0.0)
     {
      ...
      bool pos = ((a > 0.0) == (!SatSignBit(b)));
      return pos ? SatInf() : -SatInf();
     }
   return a / b;
  }
...
//------------------------------------------------------------------//
double SatSign(const double x)
  {
   if(x != x)   return SatNan();
   if(x > 0.0)  return 1.0;
   if(x < 0.0)  return -1.0;
   return 0.0;
  }
...
//------------------------------------------------------------------//
double SatBankerRound(const double x)
  {
   if(x != x)
      return SatNan();
...
//==================================================================//
// CSatEwmMean — pandas .ewm(span, adjust=True, ignore_na=False,    //
      ...
      if(m_avg == m_avg)
        {
         ...
         m_avg = cur;
      return (m_nobs >= m_minp) ? m_avg : SatNan();
     }

   double            Value() const { return (m_nobs >= m_minp) ? m_avg : SatNan(); }
  };

//==================================================================//
// CSatEwmStd — pandas .ewm(span, min_periods=minp, adjust=True,    //
//              ignore_na=False).std()                              //
         ...
         if(denominator > 0.0)
           {
            ...
            return m_clamp_neg_var ? 0.0 : SatNan();
           }
         return SatNan();
        }
      return SatNan();
...
//==================================================================//
// CSatRing — fixed-size ring of raw values (NaN allowed),          //
...
public:
   ...
   int               m_count;    // rows pushed (capped at capacity)
                     ...
                     CSatRing() { m_window = 0; m_head = 0; m_count = 0; }
...
//==================================================================//
// CSatRollStd — rolling(window).std(ddof=1, min_periods=minp),     //
// one push per bar, two-pass recompute per emission (the steppers' //
// convention).  Thin stateful wrapper over CSatRing.               //
//==================================================================//
class CSatRollStd
  {
public:
   ...
   int               m_minp;
                     ...
                     CSatRollStd() { m_window = 0; m_minp = 0; }
...
//==================================================================//
// CSatSma — pandas .rolling(window, min_periods=window).mean()     //
...
// crisis-style two-pass means live on CSatRing::Mean instead —     //
...
public:
   ...
   int               m_nan_ct;
                     ...
                     CSatSma() { m_window = 0; m_head = 0; m_filled = 0; m_sum = 0.0; m_nan_ct = 0; }
...
//==================================================================//
// CSatDonchian — rolling(w).max()/min().shift(1), min_periods=w.   //
...
public:
   ...
   long              m_n_valid;
                     ...
                     CSatDonchian() { m_w = 0; m_is_max = true; m_dq_start = 0; m_dq_len = 0; m_cap = 0; m_n_pushed = 0; m_n_valid = 0; }
...
int SatCsvSplit(const string line, string &fields[])
  {
...
double SatParseDouble(const string tok_in)
  {
   string tok = tok_in;
   StringTrimLeft(tok);
   StringTrimRight(tok);
