//+------------------------------------------------------------------+
//| Ensemble.mqh — FMA3 v34 ensemble shell (CSatEnsembleStepper)     |
//|                                                                  |
...
//+------------------------------------------------------------------+
#ifndef SAT_ENSEMBLE_MQH
#define SAT_ENSEMBLE_MQH
...
//==================================================================//
#define Sat_ENS_NSLEEVES 8

// build_c2 sleeve dict insertion order == combine() accumulation
// order: V2_CAPS key order then "mag"
const string Sat_ENS_SLEEVE_ORDER[Sat_ENS_NSLEEVES] =
  {
   "meanrev", "carry_breakout", "seasonal", "intraday",
   "crisis", "trend_v2", "crypto_smart", "mag"
  };
// WEIGHTS = {**V2_CAPS, "mag": MAG_W}   (RAW, no renormalization)
const double Sat_ENS_WEIGHTS[Sat_ENS_NSLEEVES] =
  {
   0.11, 0.046, 0.18, 0.168, 0.10, 0.042, 0.13, 0.05
  };
const int    Sat_ENS_SEASONAL_IDX = 2;    // "seasonal" slot above
const double Sat_ENS_SCALE        = 10.0; // SCALE
const double Sat_ENS_MAG_W        = 0.05; // MAG_W (== WEIGHTS[7])

// ensemble.py apply_hard_limits frozen constants
const string Sat_ENS_CROSS_SYMS[4] = {"EURCHF", "EURSEK", "EURNOK", "AUDNZD"};
const double Sat_ENS_CROSS_CAP     = 0.5;   // ABSOLUTE post-scale, all bars
const string Sat_ENS_GOLD_SYM      = "XAUUSD";
const int    Sat_ENS_OVERNIGHT_H_GE = 21;   // (hrs >= 21) | (hrs < 6)
const int    Sat_ENS_OVERNIGHT_H_LT = 6;
...
//------------------------------------------------------------------//
double SatEnsembleStructuralGoldCap()
  {
   return Sat_ENS_WEIGHTS[Sat_ENS_SEASONAL_IDX] * Sat_ENS_SCALE;
  }
...
//------------------------------------------------------------------//
bool SatEnsStrLess(const string a, const string b)
  {
   ...
   bool              AddSleeve(const string name, const string &symbols[])
     {
      if(m_finalized)
        {
         Print("CSatEnsembleStepper: AddSleeve after Finalize");
         return false;
        }
      int slot = -1;
      for(int i = 0; i < Sat_ENS_NSLEEVES; i++)
         if(Sat_ENS_SLEEVE_ORDER[i] == name) { slot = i; break; }
      if(slot < 0)
        {
         Print("CSatEnsembleStepper: unknown sleeve '", name, "'");
         return false;
        }
      for(int i = 0; i < m_nsleeves; i++)
         if(m_sleeve_ord[i] == slot)
           {
            Print("CSatEnsembleStepper: duplicate sleeve '", name, "'");
            return false;
           }
      int k = m_nsleeves;
      ...
      m_sleeve_w[k]     = Sat_ENS_WEIGHTS[slot];
   ...
   bool              Finalize()
     {
      ...
      if(m_nsleeves <= 0)
        {
         Print("CSatEnsembleStepper: Finalize with no sleeves");
         return false;
        }
      ...
      for(int p = 0; p < pool; p++)
        {
         ...
         int at = m_nsym;
         for(int q = 0; q < m_nsym; q++)
            if(SatEnsStrLess(s, m_symbols[q])) { at = q; break; }
      ...
      for(int q = 0; q < m_nsym; q++)
        {
         bool xc = false;
         for(int c = 0; c < 4; c++)
            if(m_symbols[q] == Sat_ENS_CROSS_SYMS[c]) { xc = true; break; }
         m_is_cross[q] = xc;
         m_is_gold[q]  = (m_symbols[q] == Sat_ENS_GOLD_SYM);
        }
      ...
      m_gold_cap = SatEnsembleStructuralGoldCap();
   ...
   bool              SetSleeveRow(const string name, const double &positions[])
     {
      if(!m_finalized)
        {
         Print("CSatEnsembleStepper: SetSleeveRow before Finalize");
         return false;
        }
      int i = -1;
      ...
      if(i < 0)
        {
         Print("CSatEnsembleStepper: SetSleeveRow unknown sleeve '", name, "'");
         return false;
        }
      int n = m_sl_sym_count[i];
      if(ArraySize(positions) != n)
        {
         Print("CSatEnsembleStepper: SetSleeveRow '", name, "' size ",
   ...
   bool              Step(const datetime t, double &out[])
     {
      if(!m_finalized)
        {
         Print("CSatEnsembleStepper: Step before Finalize");
         return false;
        }
      for(int i = 0; i < m_nsleeves; i++)
         if(!m_row_set[i])
           {
            Print("CSatEnsembleStepper: Step missing sleeve row '",
                  m_sleeve_name[i], "'");
            return false;
           }
      long hour = ((long)t / 3600) % 24;
      bool overnight = (hour >= Sat_ENS_OVERNIGHT_H_GE) ||
                       (hour <  Sat_ENS_OVERNIGHT_H_LT);
      ...
      for(int q = 0; q < m_nsym; q++)
        {
         ...
         acc = acc * Sat_ENS_SCALE;   // combine(...) * SCALE, post-sum
         // hard limits (frozen apply_hard_limits), post-scale.
         // NaN acc: every comparison false -> passes through, as in Python.
         if(m_is_cross[q])
           {
            if(acc > Sat_ENS_CROSS_CAP)
               acc = Sat_ENS_CROSS_CAP;
            else if(acc < -Sat_ENS_CROSS_CAP)
               acc = -Sat_ENS_CROSS_CAP;
           }
   ...
   bool              SetState(const string state)
     {
      ...
      if(np < 2 || parts[0] != "FMA3ENSv1")
        {
         Print("CSatEnsembleStepper: SetState bad header");
         return false;
        }
      Reset();
      double gc_in = SatNan();
      ...
      for(int p = 1; p < np; p++)
        {
         ...
         if(colon < 0)
           {
            Print("CSatEnsembleStepper: SetState bad sleeve entry '",
                  parts[p], "'");
            Reset();
            return false;
           }
      ...
      if(gc_seen && gc_in == gc_in && gc_in != m_gold_cap)
        {
         Print("CSatEnsembleStepper: SetState gold_cap mismatch ",
