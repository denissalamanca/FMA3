//+------------------------------------------------------------------+
//| MeanRev.mqh — FMA3 v34 meanrev sleeve stepper (CV34MeanRevStepper)|
//|                                                                  |
...
//|                                                                  |
//| SLEEVE (16 symbols, fixed order — V34MR_SYMBOLS):                |
...
//|   CV34MeanRevStepper stp;  stp.Init();                           |
//|   stp.Step(ts, closes, pos);   // ONE union-grid HOURLY bar      |
//|     ts      : server time of the union hourly bar, STRICTLY      |
//|               increasing.                                        |
//|     closes[]: >= 16 doubles in V34MR_SYMBOLS order; the RAW      |
//|               close of the bar, V34Nan() when the symbol printed |
...
//|    bar is an ewm observation (CV34EwmMean span=720, pandas       |
...
//|    division semantics (V34NpDiv); trend = px > SMA200 (false     |
...
//|    sum(|pos|) in V34MR_SYMBOLS order (order preserved), scale =  |
//|    min(1.0, 3.0/gross) via V34NpDiv (gross==0 -> inf -> 1.0).    |
//|                                                                  |
//| STATE: GetState/SetState mirror the Python get_state/set_state   |
//| dict field-for-field (SV34MeanRevState) for live EA warm-start.  |
//| cur_day is stored as the server day number ts/86400 (-1 = none), |
//| pending as a fixed FIFO of V34MR_MAX_PEND slots (the queue never |
...
//+------------------------------------------------------------------+
#ifndef FMA3V34_MEANREV_MQH
#define FMA3V34_MEANREV_MQH

#include <FMA3v34/V34Math.mqh>

#define V34MR_NSYM      16
#define V34MR_NFX       10
#define V34MR_NIDX      6
#define V34MR_RING      256   // ring capacity >= max(TREND_L, L) = 200
#define V34MR_MAX_PEND  8     // pending FIFO capacity (worst case ~2)
...
const string V34MR_SYMBOLS[V34MR_NSYM] =
  {
...
struct SV34MeanRevState
  {
   ...
   double            close[V34MR_NSYM];            // ffilled union close
   double            wavg[V34MR_NSYM];             // ewm weighted avg of ret^2
   double            old_wt[V34MR_NSYM];
   long              nobs[V34MR_NSYM];
   // --- per-symbol daily state ---
   double            dbuf[V34MR_NSYM][V34MR_RING]; // daily close rings
   int               st[V34MR_NSYM];               // hysteresis / dip state
   int               held[V34MR_NIDX];             // index holding-day counter
   double            size[V34MR_NSYM];             // frozen entry size
   // --- execution state ---
   double            pos[V34MR_NSYM];              // active hourly position
   int               pend_count;
   datetime          pend_eff[V34MR_MAX_PEND];
   double            pend_pos[V34MR_MAX_PEND][V34MR_NSYM];
  };

//==================================================================//
// CV34MeanRevStepper — steps all 16 symbols together, one HOURLY   //
// union-grid bar at a time                                         //
//==================================================================//
class CV34MeanRevStepper
  {
public:
   ...
   // --- per-symbol hourly state ---
   double            m_close[V34MR_NSYM];          // ffilled union close
   CV34EwmMean       m_ewm[V34MR_NSYM];            // ret^2 ewm (span=720)
   // --- per-symbol daily state ---
   double            m_dbuf[V34MR_NSYM][V34MR_RING];
   int               m_dptr;                       // shared write ptr
   int               m_dcount;                     // daily rows pushed
   int               m_st[V34MR_NSYM];
   int               m_held[V34MR_NIDX];           // INDICES only (i-10)
   double            m_size[V34MR_NSYM];
   // --- execution state ---
   bool              m_have_day;                   // cur_day is not None
   long              m_cur_day;                    // ts/86400
   double            m_pos[V34MR_NSYM];
   int               m_pend_count;
   datetime          m_pend_eff[V34MR_MAX_PEND];
   double            m_pend_pos[V34MR_MAX_PEND][V34MR_NSYM];

                     CV34MeanRevStepper() { Init(); }
   ...
   void              Reset()
     {
      ...
      // f = 1-alpha.  CV34EwmMean computes f = 1 - 2/(span+1) which
      ...
      for(int i = 0; i < V34MR_NSYM; i++)
        {
         ...
         for(int j = 0; j < V34MR_RING; j++)
            m_dbuf[i][j] = nan;
         m_st[i]   = 0;
         m_size[i] = 0.0;
         m_pos[i]  = 0.0;
        }
      for(int i = 0; i < V34MR_NIDX; i++)
         m_held[i] = 0;
      m_dptr       = 0;
      m_dcount     = 0;
      m_have_day   = false;
   ...
   // closes[i] is the RAW close of V34MR_SYMBOLS[i] (NaN when the   //
   ...
   void              Step(const datetime ts, const double &closes[],
                          double &pos_out[])
     {
      ...
      for(int i = 0; i < V34MR_NSYM; i++)
        {
         ...
         // CV34EwmMean::Step reproduces the meanrev kernel verbatim
      ...
      if(ArraySize(pos_out) < V34MR_NSYM)
         ArrayResize(pos_out, V34MR_NSYM);
      for(int i = 0; i < V34MR_NSYM; i++)
         pos_out[i] = m_pos[i];
     }
   ...
   //---------------------------------------------------------------//
   void              GetState(SV34MeanRevState &out) const
     {
      out.version   = 1;
      out.L         = m_L;
      out.z_in      = m_z_in;
      ...
      int n = m_pend_count;
      if(n > V34MR_MAX_PEND)
         n = V34MR_MAX_PEND;                       // never happens (<~2)
      out.pend_count = n;
      for(int k = 0; k < n; k++)
        {
   ...
   void              SetState(const SV34MeanRevState &in)
     {
      m_L         = in.L;
      m_z_in      = in.z_in;
      m_z_out     = in.z_out;
...
private:
   ...
   void              FinalizeDay(const long day)
     {
      double px[V34MR_NSYM];
      double vol[V34MR_NSYM];
      ...
      for(int i = 0; i < V34MR_NSYM; i++)
         m_dbuf[i][ptr] = px[i];
      m_dptr = (ptr + 1) % V34MR_RING;
      m_dcount++;

      int st_prev[V34MR_NSYM];
      for(int i = 0; i < V34MR_NSYM; i++)
         st_prev[i] = m_st[i];
            ...
            if(!has_nan)
              {
               ...
               zt = V34NpDiv(px[i] - mean, sd);
              }
           }
         int st = m_st[i];
         if(V34IsFinite(zt))                       // inf (sd==0) skipped
           {
            if(st == 0)
              {
               if(zt > m_z_in)
      ...
      for(int gi = V34MR_NFX; gi < V34MR_NSYM; gi++)
        {
         int hi = gi - V34MR_NFX;                  // held index
         double zt = V34Nan();
            ...
            if(p5 == p5 && px[gi] == px[gi])
              {
               ...
               zt = V34NpDiv(pct, vol[gi] * m_sqrt_d);
              }
           }
      ...
      double gross = 0.0;
      for(int i = 0; i < V34MR_NSYM; i++)          // V34MR_SYMBOLS order
         gross += MathAbs(pos_c[i]);
      double scale = V34NpDiv(m_gross_cap, gross);
      ...
      if(m_pend_count < V34MR_MAX_PEND)            // never binds (<~2)
        {
         int k = m_pend_count;
         m_pend_eff[k] = eff;
         for(int i = 0; i < V34MR_NSYM; i++)
