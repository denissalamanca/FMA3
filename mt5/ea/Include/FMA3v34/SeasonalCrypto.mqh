...
//| <FMA3v34/V34Math.mqh> (CV34EwmMean / CV34EwmStd / CV34Sma /      |
//| V34NpDiv), which are themselves verbatim ports of the same       |
//| Python primitives.                                               |
//|                                                                  |
...
//|   CV34EwmMean.m_nobs = (sea_weighted==sea_weighted) ? 1 : 0,     |
...
//+------------------------------------------------------------------+
#ifndef FMA3V34_SEASONALCRYPTO_MQH
#define FMA3V34_SEASONALCRYPTO_MQH
...
const long   V34_SC_HOUR_NS = 3600000000000;         // HOUR_NS
const long   V34_SC_DAY_NS  = 86400000000000;        // DAY_NS

// ---- seasonal frozen params (sleeves/seasonal.py) ----------------
const string V34_SC_SEA_SYMBOL       = "XAUUSD";
const int    V34_SC_SEA_ENTRY_HOUR   = 23;
const int    V34_SC_SEA_END_HOUR     = 6;
const double V34_SC_SEA_KAPPA        = 0.15;
const double V34_SC_SEA_VOL_FLOOR    = 0.05;
const int    V34_SC_SEA_SPAN_DAYS    = 30;
const double V34_SC_SEA_BARS_PER_DAY = 24.0;
const int    V34_SC_SEA_SPAN         = 720;          // int(30 * 24.0)
const double V34_SC_SEA_ANN          = 24.0 * 365.25;// SEA_BARS_PER_DAY*365.25

// ---- crypto_smart frozen params (sleeves/crypto_smart.py) --------
const int    V34_SC_NCR        = 3;
const string V34_SC_CR_SYMBOLS[3] = {"BTCUSD", "ETHUSD", "SOLUSD"};
const int    V34_SC_L_MOM      = 28;
const double V34_SC_Z_LONG     = 0.75;
const double V34_SC_Z_SHORT    = 0.25;
const double V34_SC_F_EXIT     = 0.35;
const int    V34_SC_MA_REGIME  = 120;
const double V34_SC_VOL_BUDGET = 0.065;
const int    V34_SC_VOL_SPAN_D = 30;
const double V34_SC_CAP        = 0.5;
const int    V34_SC_TRADE_LAG_H = 9;

const string V34_SC_SYMBOLS[4] = {"XAUUSD", "BTCUSD", "ETHUSD", "SOLUSD"};
...
string V34ScJNum(const double x)
  {
   if(x != x)
      return "NaN";
   double inf = V34Inf();
...
double V34ScTokD(const string t)
  {
   if(t == "NaN" || t == "nan")
      return V34Nan();
   if(t == "Infinity" || t == "inf")
...
//==================================================================//
// CV34ScCoin — per-coin daily-grid state (== _CoinState):          //
...
public:
   ...
   CV34EwmStd        m_ewm;        // _EwmStd(VOL_SPAN_D, VOL_SPAN_D), var<0 -> NaN
   ...
   CV34Sma           m_ma;         // == python ma_ring/head/filled/sum/nan_ct
   int               m_state;
   ...
   void              Init()
     {
      m_ewm.Init(V34_SC_VOL_SPAN_D, V34_SC_VOL_SPAN_D, false); // NaN flavor
      m_prev_logp = V34Nan();
      ArrayResize(m_logp_ring, 0);
      m_logp_n = 0;
      m_ma.Init(V34_SC_MA_REGIME);
      m_state = 0;
      m_last_sig_d = V34Nan();
      m_last_z     = V34Nan();
      m_last_ma    = V34Nan();
   ...
   double            StepDay(const double close)
     {
      ...
      m_logp_ring[rn] = logp;
      if(rn + 1 > V34_SC_L_MOM)                    // python list pop(0)
        {
         for(int i = 0; i < rn; i++)
      ...
      // so a zero divisor yields +-inf (V34NpDiv), never an MQL5 zero-divide.
      double z = (d28 == d28 && sig_d == sig_d)
                 ? V34NpDiv(d28, sig_d * MathSqrt((double)V34_SC_L_MOM))
                 : V34Nan();
      // ma = D.rolling(MA_REGIME, min_periods=MA_REGIME).mean()
      // (CV34Sma is the verbatim port of this exact ring+running-sum block)
      ...
      bool ok = (V34IsFinite(z) && V34IsFinite(ma));
      int state = m_state;
      if(!ok)
         state = 0;
      else
         ...
         if(state == 0)
           {
            if(z >= V34_SC_Z_LONG)
               state = 1;
            else if(z <= -V34_SC_Z_SHORT && !ab)
               state = -1;
           }
         else if(state == 1)
           {
            if(z < V34_SC_F_EXIT * V34_SC_Z_LONG)
              {
               state = 0;
               if(z <= -V34_SC_Z_SHORT && !ab)
                  state = -1;
              }
           }
         else // state == -1
           {
            if(z > -V34_SC_F_EXIT * V34_SC_Z_SHORT || ab)
              {
               state = 0;
               if(z >= V34_SC_Z_LONG)
                  state = 1;
      ...
      if(sig_d == sig_d)
        {
         ...
         w = V34NpDiv(V34_SC_VOL_BUDGET, sig_ann);   // inf if sig_ann == 0
         if(w > V34_SC_CAP)                          // np.minimum(CAP, inf) == CAP
            w = V34_SC_CAP;
         if(!V34IsFinite(w))                         // np.where(isfinite(w), w, 0)
            w = 0.0;
        }
      else
         w = 0.0;
   ...
   string            GetStateJson()
     {
      string s = "{\"ewm\": {";
      s += "\"mean\": "     + V34ScJNum(m_ewm.m_mean);
      s += ", \"cov\": "    + V34ScJNum(m_ewm.m_cov);
      s += ", \"sum_wt\": " + V34ScJNum(m_ewm.m_sum_wt);
      s += ", \"sum_wt2\": " + V34ScJNum(m_ewm.m_sum_wt2);
      s += ", \"old_wt\": " + V34ScJNum(m_ewm.m_old_wt);
      s += ", \"nobs\": "   + IntegerToString(m_ewm.m_nobs) + "}";
      s += ", \"prev_logp\": " + V34ScJNum(m_prev_logp);
      s += ", \"logp_ring\": [";
      int rn = ArraySize(m_logp_ring);
      for(int i = 0; i < rn; i++)
         s += (i > 0 ? ", " : "") + V34ScJNum(m_logp_ring[i]);
      s += "]";
      s += ", \"logp_n\": " + IntegerToString(m_logp_n);
      s += ", \"ma_ring\": [";
      for(int i = 0; i < m_ma.m_window; i++)
         s += (i > 0 ? ", " : "") + V34ScJNum(m_ma.m_buf[i]);
      s += "]";
      s += ", \"ma_head\": "   + IntegerToString(m_ma.m_head);
      s += ", \"ma_filled\": " + IntegerToString(m_ma.m_filled);
      s += ", \"ma_sum\": "    + V34ScJNum(m_ma.m_sum);
   ...
   bool              SetStateJson(CV34ScJsonTok &tk)
     {
      Init();
      if(!tk.Eat('{'))
         return false;
      ...
      while(true)
        {
         ...
         if(key == "ewm")
           ...
           {
            if(!V34ScParseDArr(tk, m_logp_ring))
               return false;
           }
         else if(key == "logp_n")
            m_logp_n = tk.L();
...
//==================================================================//
// CV34SeasonalCryptoStepper — == ConsolidateP1cStepper.            //
...
// last row.                                                        //
//==================================================================//
class CV34SeasonalCryptoStepper
  {
public:
   // --- seasonal ewm(720) of ret^2, pandas weighted/old_wt form ---
   // CV34EwmMean(span=720, minp=1) IS this recurrence verbatim
   ...
   CV34ScCoin        m_coins[3];       // BTCUSD, ETHUSD, SOLUSD
   ...
   double            m_prev_cr_row[3];
                     ...
                     CV34SeasonalCryptoStepper() { Init(); }

   void              Init()
     {
      m_sea.Init((double)V34_SC_SEA_SPAN, 1);
      m_sea_w   = 0.0;
      m_sea_vol = V34Nan();
      for(int k = 0; k < V34_SC_NCR; k++)
        {
         m_coins[k].Init();
         m_day_last[k]    = V34Nan();
         m_cr_current[k]  = V34Nan();
         m_prev_cr_row[k] = 0.0;
        }
      m_has_cur_day = false;
      m_cur_day     = 0;
   ...
   bool              StepNs(const long ts_ns, const double xau_ret,
     ...
     {
      int hour = (int)((ts_ns / V34_SC_HOUR_NS) % 24);
      bool emitted = false;
      if(m_have_prev)
        {
         double hold_next = (hour == V34_SC_SEA_ENTRY_HOUR
                             || hour < V34_SC_SEA_END_HOUR) ? 1.0 : 0.0;
      ...
      long day_idx = ts_ns / V34_SC_DAY_NS;
      if(!m_has_cur_day)
        {
         m_cur_day = day_idx;
         m_has_cur_day = true;
      ...
      double cr_row[3];
      for(int k = 0; k < V34_SC_NCR; k++)                     // .fillna(0.0)
      ...
      double vol = (var == var) ? MathSqrt(var * V34_SC_SEA_ANN) : V34Nan();
      m_sea_vol = vol;
      if(vol == vol)
        {
         double vc = (vol > V34_SC_SEA_VOL_FLOOR)
                     ? vol : V34_SC_SEA_VOL_FLOOR;            // clip(lower)
         double w = V34_SC_SEA_KAPPA / vc;                    // vc >= 0.05 > 0
         if(w > 1.0)                                          // clip(upper)
            w = 1.0;
         m_sea_w = w;
        }
   ...
   string            GetState()
     {
      string s = "{";
      s += "\"sea_weighted\": " + V34ScJNum(m_sea.m_avg);
      s += ", \"sea_old_wt\": " + V34ScJNum(m_sea.m_old_wt);
      s += ", \"sea_w\": "      + V34ScJNum(m_sea_w);
      s += ", \"coins\": {";
      for(int k = 0; k < V34_SC_NCR; k++)
         s += (k > 0 ? ", " : "") + "\"" + V34_SC_CR_SYMBOLS[k] + "\": "
      ...
      for(int i = 0; i < qn; i++)
        {
         s += (i > 0 ? ", " : "") + "[" + IntegerToString(m_q_eff[i]) + ", ["
              + V34ScJNum(m_q_row[3 * i])     + ", "
              + V34ScJNum(m_q_row[3 * i + 1]) + ", "
              + V34ScJNum(m_q_row[3 * i + 2]) + "]]";
        }
      s += "]";
      s += ", \"cr_current\": ["
           + V34ScJNum(m_cr_current[0]) + ", "
           + V34ScJNum(m_cr_current[1]) + ", "
           + V34ScJNum(m_cr_current[2]) + "]";
      s += ", \"have_prev\": ";
      s += (m_have_prev ? "true" : "false");
      s += ", \"prev_ts\": " + IntegerToString(m_prev_ts);
      s += ", \"prev_cr_row\": ["
           + V34ScJNum(m_prev_cr_row[0]) + ", "
           + V34ScJNum(m_prev_cr_row[1]) + ", "
           + V34ScJNum(m_prev_cr_row[2]) + "]";
      s += "}";
      return s;
     }
      ...
      while(true)
        {
         ...
         if(key == "sea_weighted")
           ...
           {
            double a[];
            if(!V34ScParseDArr(tk, a) || ArraySize(a) != 3)
               return false;
            for(int k = 0; k < V34_SC_NCR; k++)
               m_cr_current[k] = a[k];
           }
      ...
      if(anyv)
        {
         ...
         long eff = day_idx * V34_SC_DAY_NS + V34_SC_DAY_NS
                    + (long)(V34_SC_TRADE_LAG_H - 1) * V34_SC_HOUR_NS;
         QueuePush(eff, row);
        }
      for(int k = 0; k < V34_SC_NCR; k++)
         m_day_last[k] = V34Nan();
...
#endif // FMA3V34_SEASONALCRYPTO_MQH
