...
//|   (has_cur_day,cur_day) + SV34IntradaySymState[] in m_symbols    |
...
//+------------------------------------------------------------------+
#ifndef FMA3V34_INTRADAY_MQH
#define FMA3V34_INTRADAY_MQH

#include <FMA3v34/V34Math.mqh>

// integer nanosecond grid constants (Python DAY_NS / HOUR_NS)
const long V34_INTRADAY_DAY_NS  = 86400000000000;   // 86_400_000_000_000
const long V34_INTRADAY_HOUR_NS = 3600000000000;    //  3_600_000_000_000

//------------------------------------------------------------------//
// per-symbol state — field-for-field mirror of _new_sym_state()    //
//------------------------------------------------------------------//
struct SV34IntradaySymState
  {
...
//------------------------------------------------------------------//
// CV34IntradayStepper — steps BOTH sleeve symbols together, one    //
// union-grid hourly bar at a time (IntradayStepper 1:1).           //
//------------------------------------------------------------------//
class CV34IntradayStepper
  {
public:
   ...
   SV34IntradaySymState m_st[];

                     CV34IntradayStepper() { m_nsym = 0; m_has_day = false; m_cur_day = 0; }
   ...
   void              GetState(bool &has_cur_day, long &cur_day,
                              SV34IntradaySymState &syms[]) const
     {
   ...
   void              SetState(const bool has_cur_day, const long cur_day,
                              const SV34IntradaySymState &syms[])
     {
   ...
   void              StepNs(const long ts_ns, const double &closes[],
                            double &pos_out[])
     {
      long day  = ts_ns / V34_INTRADAY_DAY_NS;    // ts_ns >= 0 on the grid
      long hour = (ts_ns - day * V34_INTRADAY_DAY_NS) / V34_INTRADAY_HOUR_NS;
      if(!m_has_day)
        {
         m_cur_day = day;
         m_has_day = true;
      ...
      for(int i = 0; i < m_nsym; i++)
        {
         double raw = (i < ncl) ? closes[i] : V34Nan();   // closes.get(s, NAN)
         bool has_bar = V34IsObs(raw);
         ...
         if(has_bar)
           {
            if(hour == m_entry_hour - 1)
              ...
              {
               m_st[i].has16 = true;
               double mv = m_st[i].has15 ? (raw / m_st[i].c15 - 1.0) : V34Nan();
               ...
               if(m_st[i].sc_nobs >= m_sc_min_periods && m_st[i].sc_den > 0.0)
                  ...
                  sc = V34Nan();
               double z_raw = (V34IsObs(mv) && V34IsObs(sc)) ? V34NpDiv(mv, sc)
                                                             : V34Nan();
               double z;
               if(V34IsObs(z_raw))
                 {
               ...
               if(V34IsObs(wv))
                 {
                  double q = V34NpDiv(m_ref_vol, wv);   // inf when wv == 0
                  w = (q <= 1.0) ? q : 1.0;             // clip(upper=1.0)
                 }
               else
                  w = V34Nan();
               double sig;
               if(V34IsObs(z) && V34IsObs(w))
                 {
...
private:
   //----------------------------------------------------------- internals
   void              ResetSymState(SV34IntradaySymState &st)
     {
      double nan = V34Nan();
      st.prev_close = nan;
      st.vol_num    = 0.0;
