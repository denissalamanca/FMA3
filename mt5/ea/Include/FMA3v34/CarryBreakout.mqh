...
//|                                                                  |
//| NUMERIC CAVEAT (V34Math header): pandas' ewma kernel uses an     |
...
//+------------------------------------------------------------------+
#ifndef FMA3V34_CARRYBREAKOUT_MQH
#define FMA3V34_CARRYBREAKOUT_MQH
...
//==================================================================//
#define V34CB_N_FX        21
#define V34CB_N_BK        11
#define V34CB_N_SYM       32
#define V34CB_N_CCY       10
#define V34CB_N_RATE      178
#define V34CB_GATE_DAYS   63
#define V34CB_DC_CAP      64          // GATE_DAYS + 1
#define V34CB_VOL_SPAN_BARS 720       // 30d * 24
#define V34CB_ATR_SPAN_BARS 480       // 20d * 24
#define V34CB_N_FAST_BARS 480         // 20d * 24
#define V34CB_N_SLOW_BARS 960         // 40d * 24
#define V34CB_X_FAST_BARS 192         // max(5, round(0.4*20)) * 24
#define V34CB_X_SLOW_BARS 384         // max(5, round(0.4*40)) * 24

const double V34CB_SWAP_MARKUP  = 1.2;
const double V34CB_RISK_PER_POS = 0.02;
const double V34CB_VOL_FLOOR    = 0.05;
const double V34CB_W_CARRY      = 1.35;
const double V34CB_W_BK         = 2.05;
const double V34CB_GROSS_CAP    = 3.0;
const double V34CB_CARRY_THR    = 0.5;
const int    V34CB_TOP_K        = 5;
const double V34CB_M_ATR        = 3.0;

//==================================================================//
// universe (source order — CARRY_UNIV == core.FX, BK_UNIV literal) //
//==================================================================//
const string V34CB_SYMBOLS[V34CB_N_SYM] =
  {
...
//==================================================================//
const string V34CB_CCY[V34CB_N_CCY] =
   {"USD", "EUR", "GBP", "JPY", "CHF", "AUD", "NZD", "CAD", "NOK", "SEK"};
const int    V34CB_RATE_OFF[V34CB_N_CCY + 1] =
   {0, 20, 38, 61, 65, 77, 97, 120, 141, 161, 178};
const int    V34CB_RATE_DAY[V34CB_N_RATE] =
  {
...
const double V34CB_RATE_VAL[V34CB_N_RATE] =
  {
...
//==================================================================//
string V34CbJsonNum(const double x)
  {
   if(x != x)
      return "NaN";
   double inf = V34Inf();
...
//==================================================================//
// CV34CbJsonReader — minimal cursor parser for the state schema    //
...
public:
   ...
   bool              ParseNumber(double &val)
     {
      SkipWs();
      if(TryMatch("NaN"))         { val = V34Nan();  return true; }
      if(TryMatch("Infinity"))    { val = V34Inf();  return true; }
      if(TryMatch("-Infinity"))   { val = -V34Inf(); return true; }
      int start = m_i;
      while(m_i < m_n)
        {
      ...
      if(m_state == 0)
         ...
         if(c > hi)
           {
            ...
            double mv = vol;
            if(V34CB_VOL_FLOOR > mv)
               mv = V34CB_VOL_FLOOR;
            double sz = V34CB_RISK_PER_POS / mv;   // mv >= 0.05 or NaN: safe
            if(1.0 < sz)
               sz = 1.0;
            m_size = sz;
           }
      ...
      else
        {
         if(m_best < c)                 // best = max(best, c) (NaN-free here)
            m_best = c;
         if((xlo == xlo && c < xlo) || c < m_best - V34CB_M_ATR * atr)
           {
            m_state = 0;
            m_size  = 0.0;
           }
...
//==================================================================//
// CV34CarryBreakoutStepper — steps ALL 32 sleeve symbols together, //
// one union hourly bar per call.  See file header for the API and  //
// state contract.                                                  //
//==================================================================//
class CV34CarryBreakoutStepper
  {
public:
   ...
   long              m_cur_day;
   double            m_c_ff[V34CB_N_SYM];        // ffilled closes
   CV34EwmMean       m_vol_ewm[V34CB_N_SYM];     // span 720
   bool              m_vol_started[V34CB_N_SYM]; // Python _Ewm.started
   CV34EwmMean       m_atr_ewm[V34CB_N_BK];      // span 480
   bool              m_atr_started[V34CB_N_BK];
   CV34Donchian      m_win_hi_f[V34CB_N_BK];     // 480 max
   CV34Donchian      m_win_hi_s[V34CB_N_BK];     // 960 max
   CV34Donchian      m_win_lo_f[V34CB_N_BK];     // 192 min
   CV34Donchian      m_win_lo_s[V34CB_N_BK];     // 384 min
   CV34CbDonchianSys m_sys_f[V34CB_N_BK];
   CV34CbDonchianSys m_sys_s[V34CB_N_BK];
   double            m_dc_hist[V34CB_DC_CAP][V34CB_N_FX];
   int               m_dc_len;                   // <= 64 daily FX close rows
   double            m_w_eff[V34CB_N_FX];        // effective carry w (pre 1.35)

   // --- derived (rebuilt by Init, not serialized) ---
   int               m_pair_base[V34CB_N_FX];    // ccy index of s[:3]
   int               m_pair_quote[V34CB_N_FX];   // ccy index of s[3:]

   // --- debug capture of the most recent carry roll (NOT state) ---
   bool              m_last_valid;
   long              m_last_day;
   double            m_last_net[V34CB_N_FX];
   double            m_last_dir[V34CB_N_FX];
   double            m_last_sig[V34CB_N_FX];
   double            m_last_w[V34CB_N_FX];

                     CV34CarryBreakoutStepper() { Init(); }
   ...
   void              Init()
     {
      ...
      for(int j = 0; j < V34CB_N_SYM; j++)
        {
         m_c_ff[j] = nan;
         m_vol_ewm[j].Init((double)V34CB_VOL_SPAN_BARS, 1);
         m_vol_started[j] = false;
        }
      for(int k = 0; k < V34CB_N_BK; k++)
        {
         m_atr_ewm[k].Init((double)V34CB_ATR_SPAN_BARS, 1);
         m_atr_started[k] = false;
         m_win_hi_f[k].Init(V34CB_N_FAST_BARS, true);
         m_win_hi_s[k].Init(V34CB_N_SLOW_BARS, true);
         m_win_lo_f[k].Init(V34CB_X_FAST_BARS, false);
         m_win_lo_s[k].Init(V34CB_X_SLOW_BARS, false);
         m_sys_f[k].Reset();
         m_sys_s[k].Reset();
        }
      m_has_cur_day = false;
      ...
      for(int j = 0; j < V34CB_N_FX; j++)
        {
         m_w_eff[j] = 0.0;
         m_pair_base[j]  = CcyIndex(StringSubstr(V34CB_SYMBOLS[j], 0, 3));
         m_pair_quote[j] = CcyIndex(StringSubstr(V34CB_SYMBOLS[j], 3, 3));
         m_last_net[j] = nan;
         m_last_dir[j] = 0.0;
         m_last_sig[j] = 0.0;
         m_last_w[j] = 0.0;
   ...
   double            Rate(const int ccy, const long day) const
     {
      if(ccy < 0)
         return V34Nan();
      int lo = V34CB_RATE_OFF[ccy];
      int hi = V34CB_RATE_OFF[ccy + 1];
      if(lo >= hi || V34CB_RATE_DAY[lo] > day)
         return V34Nan();
      while(hi - lo > 1)
        {
         int mid = (lo + hi) / 2;
         if(V34CB_RATE_DAY[mid] <= day)
            lo = mid;
         else
            hi = mid;
        }
      return V34CB_RATE_VAL[lo];
     }
   ...
   void              Step(const long epoch_day, const double &closes[],
                          double &pos[])
     {
      ...
      for(int j = 0; j < V34CB_N_SYM; j++)
        {
         ...
         if(prev != prev)
            ...
            r = V34Nan();       // unreachable per contract (see file header)
         else
           {
            r = c / prev - 1.0;
            if(r > 0.30)
         ...
         m_atr_ewm[k].Step((d == d) ? MathAbs(d) : V34Nan());
         m_atr_started[k] = true;
         double av  = m_atr_ewm[k].Value();
         double atr = (av == av) ? av * 24.0 : V34Nan();
      ...
      for(int j = 0; j < V34CB_N_SYM; j++)
         gross += MathAbs(pos[j]);
      double scale = (gross > 0.0) ? V34CB_GROSS_CAP / gross : 1.0;
      if(scale > 1.0)
         scale = 1.0;
      for(int j = 0; j < V34CB_N_SYM; j++)
        {
   ...
   void              RollDay(const long day_p)
     {
      double dc_row[V34CB_N_FX];
      for(int j = 0; j < V34CB_N_FX; j++)
         dc_row[j] = m_c_ff[j];

      // dc_hist.append(dc_row); trim to GATE_DAYS+1 rows
      int row;
      if(m_dc_len == V34CB_DC_CAP)
        {
         for(int r = 0; r < V34CB_DC_CAP - 1; r++)
            for(int j = 0; j < V34CB_N_FX; j++)
      ...
      double direction[V34CB_N_FX];
      double net[V34CB_N_FX];
      for(int j = 0; j < V34CB_N_FX; j++)
        {
         double d = Rate(m_pair_base[j], day_p) - Rate(m_pair_quote[j], day_p);
         double n = MathAbs(d) - V34CB_SWAP_MARKUP;   // fabs(NaN) = NaN
         net[j] = n;
         direction[j] = 0.0;
         if(n > V34CB_CARRY_THR)                      // false for NaN
            direction[j] = V34Sign(d);
        }

      // cross-sectional descending rank, 'average' ties, keep <= TOP_K
      // (stable insertion sort == Python stable list.sort(key=-net))
      double lv_net[V34CB_N_FX];
      int    lv_j[V34CB_N_FX];
      int    n_live = 0;
      for(int j = 0; j < V34CB_N_FX; j++)
        {
      ...
      bool have63 = (m_dc_len > V34CB_GATE_DAYS);
      double sig[V34CB_N_FX];
      for(int j = 0; j < V34CB_N_FX; j++)
        {
         sig[j] = 0.0;
         if(direction[j] == 0.0)
         ...
         if(have63)
           {
            ...
            if(denom == 0.0)
               mom = V34Nan();  // unreachable per contract (Python raises)
            else
               mom = dc_row[j] / denom - 1.0;
           }
         if(V34Sign(mom) == direction[j])     // NaN == x is false
            sig[j] = direction[j];
        }
      ...
      for(int j = 0; j < V34CB_N_FX; j++)
        {
         ...
         double vol_d = (v == v) ? MathSqrt((v * 24.0) * 365.25) : V34Nan();
         if(vol_d < V34CB_VOL_FLOOR)          // pandas clip(lower=): NaN kept
            vol_d = V34CB_VOL_FLOOR;
         double wj = sig[j] * V34CB_RISK_PER_POS / vol_d;
...
private:
   ...
   //--- JSON writers -------------------------------------------------
   string            EwmJson(const CV34EwmMean &e, const bool started) const
     {
      // Python _Ewm.state(): [weighted, old_wt, nobs, started]
      return "[" + V34CbJsonNum(e.m_avg) + ", " + V34CbJsonNum(e.m_old_wt)
             + ", " + IntegerToString(e.m_nobs) + ", "
             + (started ? "1" : "0") + "]";
     }

   string            SysJson(const CV34CbDonchianSys &d) const
     {
      // Python _DonchianSystem.snap(): [state, size, best]
      return "[" + IntegerToString(d.m_state) + ", "
             + V34CbJsonNum(d.m_size) + ", " + V34CbJsonNum(d.m_best) + "]";
     }

   string            DonchianJson(const CV34Donchian &w) const
     {
   ...
   string            DonchianArrJson(const CV34Donchian &arr[]) const
     {
      string s = "[";
      for(int k = 0; k < V34CB_N_BK; k++)
        {
   ...
   //--- JSON parsers -------------------------------------------------
   bool              ParseEwm(CV34CbJsonReader &rd, CV34EwmMean &e,
                              bool &started)
     {
      if(!rd.Expect('['))
         return false;
   ...
   bool              ParseSys(CV34CbJsonReader &rd, CV34CbDonchianSys &d)
     {
      if(!rd.Expect('['))
         return false;
      long st;
   ...
   bool              ParseDonchian(CV34CbJsonReader &rd, CV34Donchian &w)
     {
      if(!rd.Expect('{'))
         return false;
   ...
   bool              ParseEwmArray(CV34CbJsonReader &rd, CV34EwmMean &arr[],
   ...
   bool              ParseDonchianArray(CV34CbJsonReader &rd,
                                        CV34Donchian &arr[])
     {
      if(!rd.Expect('['))
         return false;
   ...
   bool              ParseSysArray(CV34CbJsonReader &rd,
                                   CV34CbDonchianSys &arr[])
     {
      if(!rd.Expect('['))
         return false;
      for(int k = 0; k < V34CB_N_BK; k++)
   ...
   bool              ParseDoubleVec(CV34CbJsonReader &rd, double &out[],
                                    const int n)
     {
      if(!rd.Expect('['))
         return false;
   ...
   bool              ParseStateField(CV34CbJsonReader &rd, const string key)
     {
      if(key == "version")
        {
         long v;
         ...
         for(int j = 0; j < V34CB_N_SYM; j++)
           {
            ...
            if(!rd.ParseString(sym))
               return false;
            if(sym != V34CB_SYMBOLS[j])     // Python: assert symbols match
               return false;
           }
         return rd.Expect(']');
        }
      ...
      if(key == "c_ff")
         return ParseDoubleVec(rd, m_c_ff, V34CB_N_SYM);
      if(key == "vol_ewm")
         return ParseEwmArray(rd, m_vol_ewm, m_vol_started, V34CB_N_SYM);
      if(key == "atr_ewm")
         return ParseEwmArray(rd, m_atr_ewm, m_atr_started, V34CB_N_BK);
      if(key == "win_hi_f")
      ...
      if(key == "w_eff")
         return ParseDoubleVec(rd, m_w_eff, V34CB_N_FX);
      return false;                     // unknown key: schema is fixed
     }
  };

#endif // FMA3V34_CARRYBREAKOUT_MQH
