...
#property version   "2.00"   // F2.00: v1 + v34 sleeve fix (cause A EurPerQuote quote-ccy, cause B joint-share base) — both inert-by-default
#property strict
...
input bool    InpV34EurQuoteFix = false; // [v2 CAUSE A] false = v1 EurPerQuote (byte-identical, v7 frozen); true = quote-currency-correct eurq for v34-ONLY symbols (revives the 7 min-lot-starved JPY/NOK/SEK legs). v7 sizing unaffected either way.
input bool    InpV34JointSizing = false; // [v2 CAUSE B] false = v34 sizes off own sub-equity e34 (byte-identical); true = v34 sizes off its joint-account share acct*E34/(w*E7+(1-w)*E34) (model fed_frac). FEDERATED path only.
...
input string  InpEURCHF  = "EURCHF";    // [v2 CAUSE A] conversion only (v34 CHF crosses)
input string  InpEURNZD  = "EURNZD";    // [v2 CAUSE A] conversion only (v34 NZD crosses)
input string  InpEURCAD  = "EURCAD";    // [v2 CAUSE A] conversion only (v34 CAD crosses)
input string  InpEURNOK  = "EURNOK";    // [v2 CAUSE A] conversion only (v34 NOK)
input string  InpEURSEK  = "EURSEK";    // [v2 CAUSE A] conversion only (v34 SEK)
...
// [F3 v2] EurPerQuote (V7Core, included first) calls these v2 helpers defined in
// Federation.mqh (included second) — same forward-decl pattern as F3_V7BookEquity.
...
int OnInit()
{
   ...
   // [F3 v2 CAUSE A] warm the v34 EUR-conversion crosses so the MID-primary path is
   // non-cold when the fix is ON. Gated -> fix-OFF Market Watch stays v1-identical (G1/G2 parity).
   if(InpV34EurQuoteFix)
     {
      string xcr[5]={InpEURCHF,InpEURNZD,InpEURCAD,InpEURNOK,InpEURSEK};
      for(int i=0;i<5;i++) SymbolSelect(xcr[i],true);
     }
